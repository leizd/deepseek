...
![版本](https://img.shields.io/badge/version-2.2.7-blue)
...
- **Evaluation Harness（可评测）**：`evals/` 提供 AI Runtime 回归评测 —— golden 数据集 + CLI runner 对 RAG Recall@K、引用准确率、工具策略、Prompt Injection 防御、Agent 录制样例、延迟与 token/USD 成本自动打分，评分核心是纯函数库 `deepseek_infra/infra/evaluation/harness.py`。v2.2.7 CI 通过 `run_offline_eval_suite.py` 统一运行稳定离线的 RAG / Tool Policy / Prompt Injection adversarial eval，写出 `evals/reports/latest.json` 与 `latest.md`，再用 `compare_eval_baseline.py` 对照 v2.2.6 baseline 生成回归证据；`run_agent_eval.py` 保持离线可跑，但暂不做 CI 必过门禁。
...
与之配套的**质量评测**在 [evals/](evals/)（全部离线可跑）：v2.2.7 起 `python evals/runners/run_offline_eval_suite.py` 会统一留下 [latest eval report](evals/reports/latest.md)，当前 baseline 为 RAG Recall@5 1.000 / Citation Accuracy 0.8333、26 个固定攻防用例的 **Tool Policy Pass Rate 1.000 / Prompt Injection Defense Pass 1.000**，以及对抗注入小语料的 `block_rate` / `false_positive_rate` / `bypass_rate` soft gate；`compare_eval_baseline.py` 会对照 [v2.2.6 baseline](evals/baselines/v2.2.6.json) 标记退化。详见 [evals/README.md](evals/README.md) 与 [docs/EVAL_REPORTS.md](docs/EVAL_REPORTS.md)。本地安全能力复现最小命令集见 [docs/SECURITY_SMOKE.md](docs/SECURITY_SMOKE.md)。
...
- [ ] Third-party A2A implementation interop（v2.2.5 已固化本地 contract smoke；真实第三方仍待实机）
- [ ] Prompt injection soft gate 升级为硬门禁（`run_injection_adversarial.py --strict` 进 CI 必过项）
...
- [evals/README.md](evals/README.md) — 评测 harness；[docs/EVAL_REPORTS.md](docs/EVAL_REPORTS.md) — 离线评测报告与回归基线；[benchmarks/README.md](benchmarks/README.md) — 基准说明。
