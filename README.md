...
![版本](https://img.shields.io/badge/version-2.2.2-blue)
...
- **MCP 原生工具中心** — 17 个本地工具以标准 MCP Server 暴露，外部 MCP server 可安全桥接
...
- **Evaluation Harness（可评测）**：`evals/` 提供 AI Runtime 回归评测 —— golden 数据集 + CLI runner 对 RAG Recall@K、引用准确率、工具策略、Prompt Injection 防御、Agent 录制样例、延迟与 token/USD 成本自动打分，评分核心是纯函数库 `deepseek_infra/infra/evaluation/harness.py`。v2.2.0 CI 把稳定离线的 `run_rag_eval.py` 与 `run_tool_eval.py` 作为 PR 必过项；`run_agent_eval.py` 保持离线可跑，但暂不做 CI 必过门禁。
...
- [docs/COMPATIBILITY.md](docs/COMPATIBILITY.md) — MCP / A2A / OpenAI 客户端与外部 server 兼容性矩阵。
