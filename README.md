...
| Chat + RAG Citation | Agent DAG Run | MCP Tool Call |
| --- | --- | --- |
| ![DeepSeek Infra 对话与 RAG 引用回链](docs/assets/01-chat.png) | ![Agent DAG 运行时](docs/assets/03-agent-dag.png) | ![MCP 工具调用](docs/assets/02-rag-citation.png) |

| Trace Waterfall | Architecture |
| --- | --- |
| ![Trace 瀑布图](docs/assets/04-trace.png) | ![架构总览](docs/assets/architecture.svg) |

> Screenshots reflect the current v2.1.6 UI. See [2-min Demo](docs/DEMO.md) for walking through each feature live.
...
- **链路追踪**：每轮普通聊天、端侧推理和多 Agent DAG 都会生成 `traceId` 写入 `.traces/`，助手消息可打开 trace waterfall 查看各节点耗时、token 与 prompt cache 命中率；`GET /trace/{trace_id}` 提供独立只读瀑布页面，`GET /api/traces/{trace_id}/export.json` 导出；`GET /metrics` 聚合成 Prometheus 指标。
...
- **Evaluation Harness（可评测）**：`evals/` 提供 AI Runtime 回归评测 —— golden 数据集 + CLI runner 对 RAG Recall@K、引用准确率、工具调用成功率、Agent 完成率、延迟与 token/USD 成本自动打分，评分核心是纯函数库 `deepseek_infra/infra/evaluation/harness.py`。`run_rag_eval.py` 对仓库自身 `docs/` 做真实但离线的检索评测（无需 API Key），详见 [evals/README.md](evals/README.md)。
...
与之配套的**质量评测**在 [evals/](evals/)（全部离线可跑）：RAG Recall@5 1.000 / Citation Accuracy 0.833、Agent 工具调用与完成率打分，以及 26 个攻防用例的 **Tool Policy Pass Rate 1.000 / Prompt Injection Defense Pass 1.000**（`python evals/runners/run_tool_eval.py`），详见 [evals/README.md](evals/README.md)。
...
- [ ] Eval CI 固化（`run_rag_eval` + `run_tool_eval` + `run_agent_eval` 全部进 CI 门禁）
