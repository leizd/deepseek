...
![版本](https://img.shields.io/badge/version-2.1.5-blue)
...
| 7 | **MCP Tool Hub** | `infra/mcp/` | 把本地工具面封装成 MCP server（JSON-RPC 2.0：`tools` / `resources` / `prompts`），Claude Desktop、Cursor 等任意 MCP 客户端可直接复用；外接 MCP server 的 client 也在这里。 |
| 8 | **A2A Agent Mesh** | `infra/agent_runtime/a2a.py` | 为每个本地 Agent 暴露 Agent Card 与 A2A 任务生命周期（`message/send`·`message/stream`·`tasks/get`·`tasks/cancel`），支持跨 Agent 委派。 |
| 9 | **Context Taint Firewall** | `infra/gateway/context_taint.py` | 上下文按来源打信任标签（网页 / 文件 / 工具结果 = 不可信），扫描注入 / 密钥外泄 / 工具指令，隔离包装不可信块，污染轮高危工具升级人工确认。 |
