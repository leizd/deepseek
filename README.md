...
![版本](https://img.shields.io/badge/version-2.2.4-blue)
![Python](https://img.shields.io/badge/python-3.10%2B-green)
![Coverage Gate](https://img.shields.io/badge/coverage%20gate-70%25-brightgreen)
...
- **Evaluation Harness（可评测）**：`evals/` 提供 AI Runtime 回归评测 —— golden 数据集 + CLI runner 对 RAG Recall@K、引用准确率、工具策略、Prompt Injection 防御、Agent 录制样例、延迟与 token/USD 成本自动打分，评分核心是纯函数库 `deepseek_infra/infra/evaluation/harness.py`。v2.2.4 CI 把稳定离线的 `run_rag_eval.py` 与 `run_tool_eval.py` 作为 PR 必过项；`run_injection_adversarial.py` 对中文、英文、Base64、Markdown hidden instruction、多轮诱导小语料输出 block / false-positive / bypass rate（report-only）；`run_agent_eval.py` 保持离线可跑，但暂不做 CI 必过门禁。
...
与之配套的**质量评测**在 [evals/](evals/)（全部离线可跑）：RAG Recall@5 1.000 / Citation Accuracy 1.000、Agent 工具调用与完成率打分、26 个固定攻防用例的 **Tool Policy Pass Rate 1.000 / Prompt Injection Defense Pass 1.000**（`python evals/runners/run_tool_eval.py`），以及对抗注入小语料的 `block_rate` / `false_positive_rate` / `bypass_rate` report-only（`python evals/runners/run_injection_adversarial.py`），详见 [evals/README.md](evals/README.md)。
...
- [ ] 一个真实第三方 Streamable HTTP MCP server 桥接实测记录
- [ ] Third-party A2A implementation interop (local external peer loopback covered in v2.2.4)
- [ ] Coverage gate 提升到 75%

### v2.4: 评测与安全
- [ ] Prompt injection 对抗评测从 report-only 升级为带阈值门禁
