...
v2.5.1 的发布主题是 **Workspace Core / 本地 AI 工作台对象模型**：正式从 Infra 质量门禁线切到产品地基，把项目空间、保存项、产物中心和导出能力统一为 Project 2.0 对象。v2.4 已经固化 coverage 80% 与 Agent Eval CI；v2.5.1 不继续堆协议/测试门禁，而是交付可被保存、追踪、打包和导出的本地工作台闭环。
...
- README 版本徽章是 `2.5.1`。
- `CHANGELOG.md` 顶部有 `## [2.5.1]` 条目。
- `Dockerfile` 示例 tag 是 `deepseek-infra:2.5.1`。
- `docs/IMPLEMENTATION_STATUS.md` 与 `evals/README.md` 的「适用版本」是 `v2.5.1`。
...
- `evals/reports/latest.json`、`agent-latest.json`、`baseline-compare-latest.json` 与 `security-latest.json` 的 `version` 是 `2.5.1`，且包含统一 metadata。
- `docs/evidence/headless-mcp-bridge.json` 可解析、版本为 `2.5.1`，且关键 MCP bridge 步骤全为 PASS。
- `docs/evidence/a2a-external-peer.json` 可解析、版本为 `2.5.1`，且关键 A2A external peer checks 全为 PASS。
- `docs/evidence/a2a-third-party-peer.json` 缺失时为 WARNING；存在时必须版本为 `2.5.1`、`peerType=third-party`、`status=PASS` 且八类 A2A checks 全 PASS。
- `docs/evidence/edge-router-smoke.json` 缺失时为 WARNING；存在时必须版本为 `2.5.1`、`status=PASS` 且四类 Edge checks 全 PASS。
- `docs/evidence/continue-dev-mcp.json` 缺失时为 WARNING；存在时必须版本为 `2.5.1`、`status=PASS` 且六类 MCP checks 全 PASS。
- `docs/evidence/openai-compatible-sdks.json` 缺失时为 WARNING；存在时必须版本为 `2.5.1`、`status=PASS` 且 LangChain/LiteLLM/LlamaIndex 关键 SDK checks 全 PASS。
- `docs/evidence/workspace-v2.5.1.json` 必须存在、版本为 `2.5.1`、`status=PASS`，且 Project / Saved Items / Artifact / Export / secret redaction checks 全 PASS。
...
```text
dist/deepseek-infra-2.5.1.zip
dist/deepseek-infra-2.5.1.zip.sha256
dist/deepseek-infra-2.5.1.manifest.json
```
...
```json
{
  ...
  "evidence": [
    ...
    "docs/evidence/workspace-v2.5.1.json",
  ...
  "artifact": "deepseek-infra-2.5.1.zip",
  "sha256": "...",
  "bytes": 1234567
}
```
...
- run: python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v2.5.1.json
- run: python scripts/preflight_release.py --version 2.5.1
...
## 11. Workspace Core Evidence（v2.5.1）

`preflight_release.py` 自 v2.5.1 起增加 `workspace_core_evidence` 硬检查。它读取 `docs/evidence/workspace-v2.5.1.json`，确认 Workspace Core 已经用离线 smoke 跑通：
...
本项是 v2.5.1 的最低交付标准，缺失或失败会让 preflight 返回 `FAIL`。刷新命令：
...
## 14. Quality Gate Evidence（v2.5.1）
...
- Workspace Core：`docs/evidence/workspace-v2.5.1.json` `status=PASS`。
...
python scripts/release.py --clean-workspace --version 2.5.1
```
