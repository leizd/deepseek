...
v2.5.9 的发布主题是 **Web Route Split Final**：提取 chat / title / conversation search / OpenAI-compatible 路由到 `routes/chat.py`，完成 #14 全部路由拆分。`server.py` 不再内联 route handler。
...
```bash
python scripts/preflight_release.py --version 2.5.9
```

检查项：

- README 版本徽章是 `2.5.9`。
- `CHANGELOG.md` 顶部有 `## [2.5.9]` 条目。
- `Dockerfile` 示例 tag 是 `deepseek-infra:2.5.9`。
- `docs/IMPLEMENTATION_STATUS.md` 与 `evals/README.md` 的「适用版本」是 `v2.5.9`。
...
- `evals/reports/latest.json`、`agent-latest.json`、`baseline-compare-latest.json` 与 `security-latest.json` 的 `version` 是 `2.5.9`，且包含统一 metadata。
- `docs/evidence/headless-mcp-bridge.json` 可解析、版本为 `2.5.9`，且关键 MCP bridge 步骤全为 PASS。
- `docs/evidence/a2a-external-peer.json` 可解析、版本为 `2.5.9`，且关键 A2A external peer checks 全为 PASS。
- `docs/evidence/a2a-third-party-peer.json` 缺失时为 WARNING；存在时必须版本为 `2.5.9`、`peerType=third-party`、`status=PASS` 且八类 A2A checks 全 PASS。
- `docs/evidence/edge-router-smoke.json` 缺失时为 WARNING；存在时必须版本为 `2.5.9`、`status=PASS` 且四类 Edge checks 全 PASS。
- `docs/evidence/continue-dev-mcp.json` 缺失时为 WARNING；存在时必须版本为 `2.5.9`、`status=PASS` 且六类 MCP checks 全 PASS。
- `docs/evidence/openai-compatible-sdks.json` 缺失时为 WARNING；存在时必须版本为 `2.5.9`、`status=PASS` 且 LangChain/LiteLLM/LlamaIndex 关键 SDK checks 全 PASS。
- `docs/evidence/workspace-v2.5.9.json` 必须存在、版本为 `2.5.9`、`status=PASS`，且 Project / Saved Items / Artifact / Export / secret redaction checks 全 PASS。
...
```text
dist/deepseek-infra-2.5.9.zip
dist/deepseek-infra-2.5.9.zip.sha256
dist/deepseek-infra-2.5.9.manifest.json
```
...
```json
{
  ...
  "evidence": [
    ...
    "docs/evidence/workspace-v2.5.9.json",
  ...
  "artifact": "deepseek-infra-2.5.9.zip",
  "sha256": "...",
  "bytes": 1234567
}
```
...
- run: python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v2.5.9.json
- run: python scripts/preflight_release.py --version 2.5.9
...
## 11. Workspace Core Evidence（v2.5.9）

`preflight_release.py` 自 v2.5.0 起增加 `workspace_core_evidence` 硬检查。它读取 `docs/evidence/workspace-v2.5.9.json`，确认 Workspace Core 已经用离线 smoke 跑通：
...
本项是 v2.5.9 的最低交付标准，缺失或失败会让 preflight 返回 `FAIL`。刷新命令：

```bash
python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v2.5.9.json
```
...
## 14. Quality Gate Evidence（v2.5.9）
...
- Workspace Core：`docs/evidence/workspace-v2.5.9.json` `status=PASS`。
...
python scripts/release.py --clean-workspace --version 2.5.9
```
