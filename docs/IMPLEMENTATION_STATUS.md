...
- **Status**：`Stable` = 已稳定多个版本、测试覆盖深；`MVP` = 功能完整可用，但落地时间短 / 兼容性矩阵未铺开；`Partial` = 核心路径可用，存在明确缺口（缺口写在备注里）。
- **Code / Tests / Demo**：✅ 完整；🟡 部分（缺口见备注）；❌ 未开始。

| # | Module | Status | Code | Tests | Demo |
| --- | --- | --- | --- | --- | --- |
| 1 | LLM Gateway | Stable | ✅ [infra/gateway/](../deepseek_infra/infra/gateway/) | ✅ | ✅ |
| 2 | Agent DAG Runtime | Stable | ✅ [infra/agent_runtime/](../deepseek_infra/infra/agent_runtime/) | ✅ | ✅ |
| 3 | Local RAG Data Layer | Stable | ✅ [infra/rag/](../deepseek_infra/infra/rag/) | ✅ | ✅ |
| 4 | Tool Calling Runtime + Policy Engine | Stable | ✅ [infra/tool_runtime/](../deepseek_infra/infra/tool_runtime/) | ✅ | ✅ |
| 5 | Observability & Trace | Stable | ✅ [infra/observability/](../deepseek_infra/infra/observability/) | ✅ | 🟡 |
| 6 | Edge-Cloud Model Router | MVP | ✅ [infra/gateway/edge_inference.py](../deepseek_infra/infra/gateway/edge_inference.py) | 🟡 | 🟡 |
...
| 9 | Context Taint Firewall | MVP | ✅ [infra/gateway/context_taint.py](../deepseek_infra/infra/gateway/context_taint.py) | ✅ | ✅ |

横切资产（不算独立模块，但支撑「可验证性」）：

| 资产 | 位置 | 状态 |
| --- | --- | --- |
| Evaluation Harness（RAG / Agent / Tool 三条评测线） | [evals/](../evals/) · 评分核心 [infra/evaluation/harness.py](../deepseek_infra/infra/evaluation/harness.py) | ✅ 全部离线可跑 |
...
| UI 截图 / Trace 瀑布图 GIF | docs/assets/ | ❌ 计划 v2.2（见 README Roadmap） |

---

## 各模块明细

### 1. LLM Gateway — Stable
...
### 4. Tool Calling Runtime + Policy Engine — Stable
...
### 5. Observability & Trace — Stable（Demo 🟡）
...
- **MVP 的原因**：协议层与安全闸门完整，但尚未对 Claude Desktop / Cursor 等外部客户端逐一跑兼容性矩阵；外部 server 桥接（client 方向的目录合并进本地 Agent 工具面）在 Roadmap v2.3。
...
- **MVP 的原因**：任务执行是「提交即后台跑」+ SSE 快照推送；产物级流式增量（artifact streaming chunks）在 Roadmap v2.3，也尚未与第三方 A2A 实现互测。

### 9. Context Taint Firewall — MVP
...
- **MVP 的原因**：检测基于确定性 pattern 族（中英），对抗性变体的系统化基准（注入语料库 + 绕过率量化）在 Roadmap v2.4。
