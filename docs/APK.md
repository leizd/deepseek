...
- `.auth-token`、文件缓存、记忆、项目和 Agent Run 写入 Android 应用私有目录，不会写到 APK 只读资源区。
...
- OCR 在 APK 内通过 ML Kit 中文文本识别运行，图片和扫描 PDF OCR 不需要手机系统额外安装 Tesseract/Poppler；扫描 PDF 页面会以更高 scale 渲染并保留像素上限保护。
