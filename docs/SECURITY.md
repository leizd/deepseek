...
PDF 会优先读取可复制文字。OCR 是可选能力，默认关闭，以避免意外的延迟和资源消耗。图片 OCR 会把 PNG、JPG、WebP、BMP、TIFF、GIF 等图片交给本地 OCR 引擎提取文字；Windows 桌面端在 Tesseract 不可用或识别失败时会调用系统 `Windows.Media.Ocr` 兜底，Android APK 走本机 ML Kit。扫描 PDF OCR 还会通过 Poppler / `pdftoppm` 把页面转为图片。HTML 会剥离脚本/样式后抽取可见文本，EPUB/PPTX 会在 ZIP 安全校验后读取文本节点。OCR 和解析结果会和其他附件文本一样写入 `.file-cache` 或 `.projects`，因此截图、照片、扫描件和项目资料都应视为本地敏感数据。
