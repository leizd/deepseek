...
PDF 会优先读取可复制文字。OCR 是可选能力，默认关闭，以避免意外的延迟和资源消耗。图片 OCR 会把 PNG、JPG、WebP、BMP、TIFF、GIF 等图片交给本地 Tesseract 提取文字；扫描 PDF OCR 还会通过 Poppler / `pdftoppm` 把页面转为图片。HTML 会剥离脚本/样式后抽取可见文本，EPUB/PPTX 会在 ZIP 安全校验后读取文本节点。OCR 和解析结果会和其他附件文本一样写入 `.file-cache` 或 `.projects`，因此截图、照片、扫描件和项目资料都应视为本地敏感数据。
...
`.gitignore` 已默认排除 `.file-cache/`、`.projects/`、`.memory/`、`.reminders/`、`.search-cache/`、`.coverage`、`.mypy_cache/`、`.ruff_cache/`、`__pycache__/`、`.idea/`、`server*.log` 和 `pytest-cache-files-*/`。发布或分享压缩包时仍应二次检查，避免把用户附件片段、项目文档库、长期记忆、提醒、日志或本地 API Key 痕迹打包出去。
