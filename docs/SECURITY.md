...
项目空间保存在 `.projects/`，其中包含项目名称、文档元数据、预览文本、chunk 文本和本地检索向量。它是持久资料库，不会被临时 `.file-cache` 清理任务自动删除；删除项目或手动删除 `.projects/{projectId}` 才会移除。项目文档可能包含长期学习资料、工作文件或隐私内容，发布、分享或备份代码时必须排除 `.projects/`。
...
`search_files` 只读取本地 `.file-cache` 和 `.projects` 的 JSON 索引。它不会扫描任意磁盘路径，也不会把文件原文发给第三方嵌入服务；检索结果仍然会随 DeepSeek 请求发送给模型，因此项目文档和附件应视为会进入本轮模型上下文。
...
PDF 会优先读取可复制文字。OCR 是可选能力，默认关闭，以避免意外的延迟和资源消耗。图片 OCR 会把 PNG、JPG、WebP、BMP、TIFF、GIF 等图片交给本地 OCR 引擎提取文字；Windows 桌面端在 Tesseract 不可用或单页识别失败时会调用系统 `Windows.Media.Ocr` 兜底，Android APK 走本机 ML Kit。`OCR_MODE` 只控制本地 OpenCV 预处理、多 `psm` 重试和轻量结构整理，不会把原始图片上传到 DeepSeek 或云端视觉服务。`OCR_FORMULA_CMD` 是用户显式配置或本机 PATH 中存在的本地命令行工具，后端会把临时图片路径传给该命令并读取 stdout；它不走 shell，但该命令本身拥有本机进程权限，应只配置可信工具。扫描 PDF OCR 还会通过 Poppler / `pdftoppm` 把页面转为图片。HTML 会剥离脚本/样式后抽取可见文本，EPUB/PPTX 会在 ZIP 安全校验后读取文本节点。OCR 和解析结果会和其他附件文本一样写入 `.file-cache` 或 `.projects`，因此截图、照片、扫描件和项目资料都应视为本地敏感数据。
...
`.gitignore` 已默认排除 `.auth-token`、`.launcher-config.json`、`.file-cache/`、`.projects/`、`.memory/`、`.reminders/`、`.search-cache/`、`.agent-runs/`、`.coverage`、`.mypy_cache/`、`.ruff_cache/`、`__pycache__/`、`.idea/`、`server*.log` 和 `pytest-cache-files-*/`。发布或分享压缩包时仍应二次检查，避免把用户附件片段、项目文档库、长期记忆、提醒、日志、Agent Run 过程或本地 API Key 痕迹打包出去。

## 已知边界

- 本地 HTTP 不提供传输层加密；局域网内其他设备理论上可能观察流量。
- Python `ThreadingHTTPServer` 适合个人本地使用，不适合作为公网服务。
