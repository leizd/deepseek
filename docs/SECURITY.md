...
- **外部 MCP server 桥接（v2.2.1）**：`MCP_CLIENT_ENABLED=1` 时，外部 server 的工具目录会以 `mcp__<server>__<tool>` 命名空间进入本地 Agent 工具面。桥接层会按 annotations / schema / 描述做保守风险推断，执行前继续经过 Tool Policy、审批和 schema 校验，执行后把结果按不可信外部输出清洗，并写入 server/tool/argsHash/latencyMs/errorType 审计字段。
