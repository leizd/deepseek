...
| Workspace Core smoke | [docs/evidence/workspace-v2.6.1.json](evidence/workspace-v2.6.1.json) / [WORKSPACE.md](WORKSPACE.md) | ✅ Offline smoke tested | `python scripts/smoke_workspace.py --offline --out docs/evidence/workspace-v2.6.1.json` |
| Skill System smoke | [docs/evidence/skills-v2.6.1.json](evidence/skills-v2.6.1.json) / [SKILLS.md](SKILLS.md) | ✅ Offline smoke tested | `python scripts/smoke_skills.py --offline --out docs/evidence/skills-v2.6.1.json` |
...
| Workspace Core | `docs/evidence/workspace-v2.6.1.json` | `status=PASS` 且关键 checks 全 PASS |
| Skill System | `docs/evidence/skills-v2.6.1.json` | `status=PASS` 且关键 checks 全 PASS |
| Runtime doctor | `python scripts/doctor.py --offline` | exit 0 |
| Release preflight | `python scripts/preflight_release.py --version 2.6.1` | exit 0 |
...
| --- | --- | --- |
| Release zip | `dist/deepseek-infra-2.6.1.zip` | 可分发源码包 |
| Checksum | `dist/deepseek-infra-2.6.1.zip.sha256` | 校验 zip 完整性 |
| Manifest | `dist/deepseek-infra-2.6.1.manifest.json` | 版本、commit、构建环境、evidence 清单与 `qualityGates` |
...
- `workspace_core_evidence`：必须存在 `docs/evidence/workspace-v2.6.1.json`，版本匹配、`status=PASS`，且项目 / 保存项 / 产物 / 对话导出 / 项目 ZIP / 脱敏 checks 全 PASS。
- `skill_system_evidence`：必须存在 `docs/evidence/skills-v2.6.1.json`，版本匹配、`status=PASS`，且 Skill API route / registry / runner / artifact / project binding checks 全 PASS。
