...
| `static/modules/skills.js` | `initSkillWorkbench`、`renderProjectSkillBinding`、`isSkillPanelOpen`、`closeSkillWorkbench` | Skill Workbench UI 的入口、schema 运行表单、导入/导出、启用/禁用、项目绑定、recent runs 与 Skill run 结果回链；通过回调接入 `chat.js` 的面板生命周期与当前项目状态。 |
