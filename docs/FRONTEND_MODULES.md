...
| `static/modules/skills.js` | `initSkillWorkbench`、`renderProjectSkillBinding`、`isSkillPanelOpen`、`closeSkillWorkbench`、`openPacksHost`、`openVersionHost`、`renderPackCard`、`importPackFromFile` | Skill Workbench UI 的入口、schema 运行表单、导入/导出、启用/禁用、项目绑定、recent runs 与 Skill run 结果回链；v2.6.6 新增 Skill Versioning / Version History 面板，用于查看 Skill / Pack revision history、diff、migration plan、rollback 与 eval-aware upgrade gate，并继续通过回调接入 `chat.js` 的面板生命周期与当前项目状态。 |
