...
| `static/modules/skills.js` | `initSkillWorkbench`、`renderProjectSkillBinding`、`isSkillPanelOpen`、`closeSkillWorkbench`、`openPacksHost`、`openVersionHost`、`openRunsHost`、`openSecurityHost`、`renderPackCard`、`importPackFromFile` | Skill Workbench UI 的入口、schema 运行表单、导入/导出、启用/禁用、项目绑定、recent runs 与 Skill run 结果回链；v2.6.6 新增 Skill Versioning / Version History 面板，v2.6.7 新增 Runs 面板用于 Skill Run Analytics，v2.6.8 新增 Security 面板用于 Skill / Pack security review、trust store、manifest preview、tool grant risk 和 block/trust actions，并继续通过回调接入 `chat.js` 的面板生命周期与当前项目状态。 |
