...
| `semanticCache` | object | 本地语义缓存状态：`enabled`、`.semantic-cache/cache.sqlite3` 路径、相似度阈值、TTL、条目数、命中数、embedding provider，以及 v2.0.6 新增的 `cacheVersion`（命名空间戳）、`minQualityScore`（低质量门控阈值）、`cacheAttachments`（是否缓存文件上下文）。 |
...
v2.0.5 起 span 形成端到端调用树（run 为根）。典型一次多 Agent 请求：
...
断点续跑（v2.0.4）。对被中断的 run（`orphaned` / `failed` / `cancelled` / `done`）从最近检查点恢复：
