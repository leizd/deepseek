...
| `ocr` | object | OCR 能力摘要：`enabled`、`mode`、`localOnly`。这里只返回配置，不探测真实 OCR 引擎，避免启动变慢。 |
