...
错误码遵循 JSON-RPC：`-32700` parse / `-32600` invalid request / `-32601` method not found / `-32602` invalid params / `-32603` internal。`GET /api/mcp` 返回 Hub 状态（协议版本、能力画像、工具数、外接 client 配置）。外接方向：`MCP_CLIENT_ENABLED=1` + `MCP_CLIENT_SERVERS` 配置后，内置 `MCPClient` 可对外部 MCP server 做 `initialize` / `tools/list` / `tools/call`。
