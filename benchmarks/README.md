...
| `bench_semantic_cache.py` | 语义缓存 store / lookup 延迟、精确命中率、改写命中率、误命中率 | 无（离线） |
...
实测样例数字与解读见 README「Benchmarks」一节。基准只测本仓库可控的部分：离线项数字逐次可复现（检索基准钉 `PYTHONHASHSEED`）；在线项受上游负载影响，应报告分位数而不是单次值。
