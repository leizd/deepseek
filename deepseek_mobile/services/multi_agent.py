...
from deepseek_mobile.core.config import DEFAULT_MODEL, MULTI_AGENT_TIMEOUT_SECONDS
...
# Planner 也用 pro 拆分质量更稳；走 stream 让用户看到 reasoning 但不再把 JSON 灌进主正文
...
def parse_structured_agent_output(text: str) -> dict[str, str]:
    ...
    - 用 `^## 标题` 切段，标题做小写归一后按别名表归类
    - 任何一段缺失就留空字符串
    - 解析完全失败时 summary 为空、full_output = 原文，调用方据此回退
    """
...
def execute_agent_tier(
    ...
    *,
    ...
    """按层执行 agent；v1.2.5 起只有 coder / reasoner 这一层真正并行。

    Researcher 必须先产出资料，Critic 必须最后复核；中间层共享同一份 prior_outputs，
    输出按 Planner 原顺序返回，避免 Leader 综合阶段因为完成顺序漂移而变得不稳定。
    """
    # v1.2.5：Researcher 和 Critic 仍按层串行，只把 coder / reasoner 中间层并行化。
    if _tier_runs_in_parallel(tier):
...
def _execute_agent_tier_parallel(
    ...
    *,
    ...
    pool = ThreadPoolExecutor(max_workers=min(len(tier), len(MIDDLE_PARALLEL_AGENT_IDS)))
    try:
...
def stream_synthesis_for_outputs(
    ...
    *,
    ...
    diagnostics = diagnostics_for_agent_run(agent_outputs, synthesizer_usage, search_budget)
...
def safe_agent_plan(plan: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for item in raw_agents:
        ...
        if agent_id not in AGENT_PROFILES or agent_id in seen:
            continue
        agents.append({"id": agent_id, "task": task or "分析用户问题并给出公开摘要"})
        seen.add(agent_id)
...
def layered_plan(plan: list[dict[str, str]]) -> list[list[dict[str, str]]]:
    """把 plan 拆成 3 层：researcher → middle (coder/reasoner) → critic。

    v1.2.5 保持层之间串行，只让 middle 层内的 coder / reasoner 并行执行。
    """
