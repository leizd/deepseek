...
def check_workspace_core_evidence(root: Path, version: str) -> CheckResult:
    path = root / "docs" / "evidence" / "workspace-v2.5.8.json"
    if not path.is_file():
        return CheckResult(
...
def check_semantic_cache_onnx_evidence(root: Path, version: str) -> CheckResult:
    path = root / "docs" / "evidence" / "semantic-cache-onnx-v2.5.8.json"
    if not path.is_file():
        return CheckResult(
            "semantic_cache_onnx_evidence",
            STATUS_WARN,
            "Semantic Cache ONNX evidence missing; run benchmarks/bench_semantic_cache.py --compare --out docs/evidence/semantic-cache-onnx-v2.5.8.json --markdown docs/evidence/semantic-cache-onnx-v2.5.8.md",
            {"path": str(path)},
        )
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
