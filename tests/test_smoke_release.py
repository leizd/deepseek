...
def test_offline_mode_runs_doctor_evals_and_agent_only() -> None:
    ...
    names = _names(stages)
    assert names == ["doctor", "workspace_core", "offline_eval_suite", "security_corpus", "agent_eval", "baseline_compare"]
...
def test_with_server_mode_includes_protocol_smokes() -> None:
    ...
    stages = mod.build_stages(args)
    names = _names(stages)
    assert names == ["doctor", "workspace_core", "offline_eval_suite", "security_corpus", "agent_eval", "baseline_compare", "mcp_smoke", "a2a_smoke"]
    doctor_cmd = stages[0][1]
    assert "--with-server" in doctor_cmd
    assert "--base-url" in doctor_cmd
    mcp_cmd = " ".join(stages[6][1])
    assert "--mcp-url" in mcp_cmd
    assert "http://127.0.0.1:9000/mcp" in mcp_cmd
    assert "tok" in mcp_cmd
    a2a_cmd = " ".join(stages[7][1])
...
def test_default_mode_is_offline() -> None:
    ...
    assert _names(mod.build_stages(args)) == ["doctor", "workspace_core", "offline_eval_suite", "security_corpus", "agent_eval", "baseline_compare"]


def test_skip_flags_drop_stages() -> None:
    mod = _load_smoke_release()
    args = mod.parse_args(["--offline", "--skip-doctor", "--skip-agent"])
    assert _names(mod.build_stages(args)) == ["workspace_core", "offline_eval_suite", "security_corpus", "baseline_compare"]


def test_with_server_skip_protocol_keeps_evals() -> None:
    mod = _load_smoke_release()
    args = mod.parse_args(["--with-server", "--skip-mcp", "--skip-a2a", "--skip-doctor"])
    assert _names(mod.build_stages(args)) == ["workspace_core", "offline_eval_suite", "security_corpus", "agent_eval", "baseline_compare"]
...
def test_json_mode_emits_plan_without_running(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert stage_names == ["doctor", "workspace_core", "offline_eval_suite", "security_corpus", "agent_eval", "baseline_compare", "mcp_smoke", "a2a_smoke"]
