...
def _skeleton(tmp_path: Path, version: str, *, release_exclusions: bool = True) -> Path:
    ...
    _write_workspace_evidence(evidence_dir / "workspace-v2.5.2.json", version)
    (root / "evals").mkdir()
...
def test_preflight_fails_on_missing_workspace_core_evidence(tmp_path: Path) -> None:
    preflight = _load_preflight()
    root = _skeleton(tmp_path, "2.5.2")
    (root / "docs" / "evidence" / "workspace-v2.5.2.json").unlink()
    result = next(r for r in preflight.run_preflight(root, "2.5.2") if r.name == "workspace_core_evidence")
    assert result.status == "fail"
...
def test_preflight_fails_on_workspace_core_missing_required_check(tmp_path: Path) -> None:
    preflight = _load_preflight()
    root = _skeleton(tmp_path, "2.5.2")
    _write_workspace_evidence(root / "docs" / "evidence" / "workspace-v2.5.2.json", "2.5.2", omit_check="projectExportZip")
    result = next(r for r in preflight.run_preflight(root, "2.5.2") if r.name == "workspace_core_evidence")
    assert result.status == "fail"
