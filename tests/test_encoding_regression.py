...
class EncodingRegressionTests(unittest.TestCase):
    ...
    def test_v244_a2a_third_party_ecosystem_evidence_patch_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        ...
        evidence = json.loads(Path("docs/evidence/a2a-third-party-peer.json").read_text(encoding="utf-8"))
        markdown = Path("docs/evidence/a2a-third-party-peer.md").read_text(encoding="utf-8")
        test_preflight = Path("tests/test_preflight_release.py").read_text(encoding="utf-8")
        test_smoke = Path("tests/test_a2a_external_peer_smoke.py").read_text(encoding="utf-8")

        self.assertIn("## [2.4.5] - A2A Third-Party Ecosystem Evidence Patch", changelog)
        self.assertIn("### v2.4.5: A2A Third-Party Ecosystem Evidence Patch", readme)
        self.assertIn("Third-party A2A ecosystem peer", compatibility)
        self.assertIn("✅ Third-party evidence tested", compatibility)
        self.assertIn("A2A third-party peer structured evidence", implementation)
        self.assertIn("A2A Third-Party Peer Evidence（v2.4.5）", readiness)
        self.assertIn("a2a_third_party_peer_evidence", evidence_index)
        self.assertIn("验证记录 + 复现流程", third_party_plan)
        self.assertIn("check_a2a_third_party_peer_evidence", preflight)
        self.assertIn("THIRD_PARTY_SCHEMA_VERSION", smoke)
        self.assertIn("--markdown", smoke)
        self.assertIn("docs/evidence/a2a-third-party-peer.json", manifest)
        self.assertEqual(schema["properties"]["checks"]["required"], list(evidence["checks"].keys()))
        self.assertEqual(evidence["schemaVersion"], "a2a-third-party-peer-evidence.v1")
        self.assertEqual(evidence["version"], "2.4.5")
        self.assertEqual(evidence["peerType"], "third-party")
        self.assertEqual(evidence["peer"]["type"], "third-party")
        self.assertEqual(evidence["status"], "PASS")
        self.assertTrue(all(value == "PASS" for value in evidence["checks"].values()))
        self.assertIn("# A2A Third-Party Peer Evidence", markdown)
        self.assertIn("test_preflight_fails_on_a2a_third_party_peer_missing_required_check", test_preflight)
        self.assertIn("test_cli_writes_third_party_json_and_markdown", test_smoke)
