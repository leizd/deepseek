...
class EncodingRegressionTests(unittest.TestCase):
    ...
    def test_v086_reasoning_timer_and_busy_interactions_are_present(self) -> None:
        ...
        self.assertIn("deepseek-infra-v187", sw)
        self.assertIn("version-2.6.9-blue", readme)
        self.assertIn("## [0.8.6]", changelog)
        self.assertIn("适用版本：v2.6.9。", api)
    ...
    def test_v170_streaming_phase_labels_and_timer_are_present(self) -> None:
        ...
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn("deepseek-infra-v187", sw)
        self.assertIn("version-2.6.9-blue", readme)
    ...
    def test_v092_upload_limits_and_frontend_interactions_are_present(self) -> None:
        ...
        for token in [
            ...
            with self.subTest(token=token):
                self.assertIn(token, status_routes)
    ...
    def test_v094_citations_titles_peek_and_timeline_are_present(self) -> None:
        ...
        client = Path("deepseek_infra/infra/gateway/deepseek_client.py").read_text(encoding="utf-8")
        tools = Path("deepseek_infra/infra/tool_runtime/tools.py").read_text(encoding="utf-8")
        chat_routes = Path("deepseek_infra/web/routes/chat.py").read_text(encoding="utf-8")
        ...
        self.assertIn('"/api/title"', chat_routes)
    ...
    def test_v096_search_hotfix_and_tool_expansion_are_present(self) -> None:
        ...
        self.assertIn("适用版本：v2.6.9。", api)
        self.assertIn("适用版本：v2.6.9。", security)
    ...
    def test_v162_android_ocr_bridge_is_present(self) -> None:
        ...
        self.assertIn('versionName "2.6.9"', build_gradle)
    ...
    def test_v166_gemini_skin_and_frontend_fixes_are_present(self) -> None:
        ...
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn('versionName "2.6.9"', build_gradle)
        self.assertIn("versionCode 269", build_gradle)
        self.assertIn("version-2.6.9-blue", readme)
    ...
    def test_v180_gateway_resiliency_is_present(self) -> None:
        ...
        trace_api = Path("deepseek_infra/infra/observability/trace_api.py").read_text(encoding="utf-8")
        ...
        self.assertIn("/api/traces/{trace_id}", trace_api)
        self.assertIn("/api/semantic-cache/status", status_routes)
        self.assertIn("/api/gateway/status", status_routes)
    ...
    def test_v200_infra_platform_reposition_is_present(self) -> None:
        ...
        # OpenAI 兼容网关
        self.assertIn("/v1/chat/completions", chat_routes)
        self.assertIn("/v1/models", chat_routes)
    ...
    def test_v201_multi_provider_ollama_is_present(self) -> None:
        ...
        self.assertIn("openai_chat_completion", chat_routes)
        self.assertIn('"providers": deps.providers_status()', status_routes)
    ...
    def test_v210_cost_and_token_budget_manager_is_present(self) -> None:
        ...
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn('"/api/budget"', status_routes)
    ...
    def test_v211_tool_policy_engine_is_present(self) -> None:
        ...
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn('"/api/tool-policy"', status_routes)
        self.assertIn("deps.tool_policy_status()", status_routes)
    ...
    def test_v213_request_scheduler_is_present(self) -> None:
        ...
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn('"/api/scheduler"', status_routes)
        self.assertIn("deps.scheduler_status()", status_routes)
    ...
    def test_v216_context_taint_firewall_is_present(self) -> None:
        ...
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn('"/api/taint"', status_routes)
    ...
    def test_v220_visualization_and_verification_assets_are_present(self) -> None:
        """v2.2.6：README 里的 Infra 叙事必须落到可点击 / 可复现 / 可部署 / 可评测的资产上。"""
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        sw = Path("static/sw.js").read_text(encoding="utf-8")
        ...
        self.assertIn("DEEPSEEK_INFRA_STATIC_DIR=/app/static", dockerfile)
        ...
        self.assertIn("python evals/runners/run_offline_eval_suite.py", ci)
        self.assertIn("python evals/runners/compare_eval_baseline.py", ci)
        self.assertIn("docker build -t deepseek-infra:test .", ci)
        ...
        # 8. 架构总览图 + v2.2 截图资产进第一屏；9. Roadmap 节
        self.assertTrue(Path("docs/assets/architecture.svg").is_file())
        self.assertIn("docs/assets/architecture.svg", readme)
        for asset in (
            "docs/assets/trace-waterfall.png",
            "docs/assets/agent-dag-run.png",
            "docs/assets/rag-citation.png",
            "docs/assets/mcp-tool-call.png",
        ):
        ...
        # 10. Trace 独立页面 + 脱敏导出 + PWA app shell
        api = Path("docs/API.md").read_text(encoding="utf-8")
        trace_api = Path("deepseek_infra/infra/observability/trace_api.py").read_text(encoding="utf-8")
        trace_export = Path("deepseek_infra/infra/observability/export.py").read_text(encoding="utf-8")
        self.assertTrue(Path("static/trace_viewer.html").is_file())
        self.assertTrue(Path("static/modules/trace_viewer.js").is_file())
        self.assertTrue(Path("static/modules/trace_waterfall.js").is_file())
        self.assertIn("/api/traces/{trace_id}/export.json", trace_api)
        self.assertIn("redacted", trace_export)
        self.assertIn("GET `/trace/{traceId}`", api)
        self.assertIn("/trace_viewer.html", sw)
        self.assertIn("/modules/trace_viewer.js", sw)
        self.assertIn("/modules/trace_waterfall.js", sw)

        # 版本联动：后端 / Android / 文档 / 工具链 / 前端缓存同步到 v2.2.6
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn("version-2.6.9-blue", readme)
        self.assertIn("## [2.2.6] - Eval Gate & Security Hardening", changelog)
        self.assertIn("deepseek-infra-v187", sw)
    ...
    def test_v221_external_mcp_bridge_is_present(self) -> None:
        ...
        self.assertIn("### v2.3: Protocol Interop GA", readme)
        self.assertIn("### v2.3.1: GUI Interop Evidence Patch", readme)
        ...
        self.assertIn("### T7 · 外部 MCP server 恶意或失联（v2.2.1，v2.2.2 加固）", threat_model)
    ...
    def test_v222_mcp_policy_hardening_is_present(self) -> None:
        """v2.2.2：外部 MCP 工具必须在所有入口共享同一个不可绕过的策略闸门。"""
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        api = Path("docs/API.md").read_text(encoding="utf-8")
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        ...
        threat_model = Path("docs/THREAT_MODEL.md").read_text(encoding="utf-8")
        status_doc = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        permissions = Path("deepseek_infra/infra/mcp/permissions.py").read_text(encoding="utf-8")
        executor = Path("deepseek_infra/infra/mcp/executor.py").read_text(encoding="utf-8")
        bridge = Path("deepseek_infra/infra/mcp/bridge.py").read_text(encoding="utf-8")
        tool_policy = Path("deepseek_infra/infra/tool_runtime/tool_policy.py").read_text(encoding="utf-8")
        tools = Path("deepseek_infra/infra/tool_runtime/tools.py").read_text(encoding="utf-8")
        test_mcp = Path("tests/test_mcp.py").read_text(encoding="utf-8")
        test_tool_policy = Path("tests/test_tool_policy.py").read_text(encoding="utf-8")

        self.assertLess(
            changelog.index("## [2.2.2] - MCP Policy Hardening"),
            changelog.index("## [2.2.1] - External MCP Tool Bridge"),
        )
        self.assertIn("version-2.6.9-blue", readme)
        self.assertIn("### v2.2.2: MCP Policy Hardening", readme)
        self.assertIn("/mcp tools/call", api)
        self.assertIn("upstream_tool_error", api)
        self.assertIn("远端 `isError=true`", compatibility)
        self.assertIn("v2.2.2 加固", security)
        self.assertIn("通用参数扫描（v2.2.2）", threat_model)
        self.assertIn("远端 `isError=true`", status_doc)

        self.assertIn("metadata_provider=external_mcp_registry.metadata_provider", permissions)
        self.assertIn("External MCP tools require ToolPolicy", executor)
        self.assertIn("decision = policy.evaluate", executor)
        self.assertIn("upstream_tool_error", executor)
        self.assertIn('error_type = "tool_error"', executor)
        self.assertIn("evaluate_network_argument_safety", tool_policy)
        self.assertIn("_URL_ARGUMENT_KEYS", tool_policy)
        self.assertIn("_PATH_ARGUMENT_KEYS", tool_policy)
        self.assertIn("schema_for_tool", tools)
        self.assertIn("external_mcp_registry.refresh()", tools)
        self.assertIn("_collision_safe_profile", bridge)
        self.assertIn("sha256", bridge)

        for test_name in (
            "test_external_mcp_hub_call_denies_without_approval",
            "test_external_mcp_tool_error_is_not_wrapped_as_success",
            "test_external_mcp_schema_lookup_is_dynamic_after_local_cache_is_built",
            "test_agent_tool_definitions_refreshes_external_registry",
            "test_external_mcp_sanitized_name_collision_gets_hash_suffix",
        ):
            with self.subTest(test_name=test_name):
                self.assertIn(test_name, test_mcp)
        self.assertIn("test_external_network_tools_scan_url_like_arguments_for_ssrf", test_tool_policy)
        self.assertIn("test_external_filesystem_tools_scan_path_like_arguments", test_tool_policy)

    def test_v224_a2a_artifact_streaming_and_interop_is_present(self) -> None:
        a2a = Path("deepseek_infra/infra/agent_runtime/a2a.py").read_text(encoding="utf-8")
        metrics = Path("deepseek_infra/infra/observability/metrics.py").read_text(encoding="utf-8")
        observability = Path("deepseek_infra/infra/observability/observability.py").read_text(encoding="utf-8")
        test_a2a = Path("tests/test_a2a.py").read_text(encoding="utf-8")
        ...
        self.assertIn("a2a_tasks_total", observability)

        for test_name in (
            "test_tasks_resubscribe_replays_artifact_chunks_after_cursor",
            "test_a2a_client_stream_roundtrip_against_local_mesh",
            "test_a2a_trace_and_prometheus_metrics",
        ):
            self.assertIn(test_name, test_a2a)

        self.assertTrue(Path("examples/a2a_peer_demo.py").is_file())
        self.assertIn("### v2.2.4: A2A Artifact Streaming & Agent Interop", readme)
        self.assertIn("## [2.2.4] - A2A Artifact Streaming & Agent Interop", changelog)
        self.assertIn("tasks/resubscribe", api)
        self.assertIn("artifactChunks", api)
        self.assertIn("A2A Agent Mesh", status_doc)
        self.assertIn("MVP", status_doc)
        self.assertIn("Local external A2A peer loopback", compatibility)
        self.assertIn("A2A MVP Acceptance", compatibility)

    def test_v225_compatibility_smoke_pack_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        status_doc = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        api = Path("docs/API.md").read_text(encoding="utf-8")
        edge_runbook = Path("docs/EDGE_ROUTER_RUNBOOK.md").read_text(encoding="utf-8")
        test_contract = Path("tests/test_a2a_compat_contract.py").read_text(encoding="utf-8")

        self.assertTrue(Path("scripts/smoke_mcp_compat.py").is_file())
        self.assertTrue(Path("scripts/smoke_a2a_compat.py").is_file())
        self.assertTrue(Path("examples/a2a_compat_smoke.py").is_file())
        self.assertTrue(Path("examples/edge_router_smoke.py").is_file())
        self.assertIn("version-2.6.9-blue", readme)
        self.assertIn("### v2.2.5: Compatibility Smoke Pack", readme)
        self.assertIn("## [2.2.5] - Compatibility Smoke & Release Polish", changelog)
        self.assertIn("Compatibility Smoke Pack", compatibility)
        self.assertIn("scripts/smoke_mcp_compat.py --external-server-url <url>", compatibility)
        self.assertIn("tests/test_a2a_compat_contract.py", compatibility)
        self.assertIn("Compatibility Smoke Pack", status_doc)
        self.assertIn("smoke_mcp_compat.py", api)
        self.assertIn("smoke_a2a_compat.py", api)
        self.assertIn("OLLAMA_ENABLED=1", edge_runbook)
        self.assertIn("test_a2a_compat_contract_card_send_stream_and_resubscribe", test_contract)

    def test_v227_eval_reports_regression_evidence_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        evals_readme = Path("evals/README.md").read_text(encoding="utf-8")
        eval_reports = Path("docs/EVAL_REPORTS.md").read_text(encoding="utf-8")
        status_doc = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        suite_runner = Path("evals/runners/run_offline_eval_suite.py").read_text(encoding="utf-8")
        compare_runner = Path("evals/runners/compare_eval_baseline.py").read_text(encoding="utf-8")
        baseline = Path("evals/baselines/v2.2.6.json").read_text(encoding="utf-8")
        test_harness = Path("tests/test_eval_harness.py").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        build_gradle = Path("android/app/build.gradle").read_text(encoding="utf-8")

        self.assertTrue(Path("evals/reports/latest.json").is_file())
        self.assertTrue(Path("evals/reports/latest.md").is_file())
        self.assertIn("version-2.6.9-blue", readme)
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn('versionName "2.6.9"', build_gradle)
        self.assertIn("versionCode 269", build_gradle)
        self.assertIn("## [2.2.7] - Eval Reports & Regression Evidence", changelog)
        self.assertIn("### v2.2.7: Eval Reports & Regression Evidence", readme)
        self.assertIn("run_offline_eval_suite.py", evals_readme)
        self.assertIn("compare_eval_baseline.py", evals_readme)
        self.assertIn("offline-eval-suite.v1", suite_runner)
        self.assertIn("offline-eval-compare.v1", compare_runner)
        self.assertIn('"version": "2.2.6"', baseline)
        self.assertIn("Eval Reports", eval_reports)
        self.assertIn("baseline compare artifact", status_doc)
        self.assertIn("run_offline_eval_suite.py --include-agent --strict --out evals/reports/latest.json", ci)
        self.assertIn("actions/upload-artifact@v4", ci)
        self.assertIn("offline-eval-report", ci)
        self.assertIn("test_offline_eval_suite_builds_json_schema_and_markdown", test_harness)
        self.assertIn("test_eval_baseline_compare_pass_warning_and_fail_paths", test_harness)

    def test_v228_agent_eval_replay_stability_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        evals_readme = Path("evals/README.md").read_text(encoding="utf-8")
        eval_reports = Path("docs/EVAL_REPORTS.md").read_text(encoding="utf-8")
        agent_eval = Path("docs/AGENT_EVAL.md").read_text(encoding="utf-8")
        status_doc = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        runner = Path("evals/runners/run_agent_eval.py").read_text(encoding="utf-8")
        suite_runner = Path("evals/runners/run_offline_eval_suite.py").read_text(encoding="utf-8")
        normalizer = Path("deepseek_infra/infra/evaluation/agent_recording.py").read_text(encoding="utf-8")
        schema = json.loads(Path("evals/schemas/agent_prediction.schema.json").read_text(encoding="utf-8"))
        baseline = json.loads(Path("evals/baselines/agent-v2.2.8.json").read_text(encoding="utf-8"))
        agent_report = json.loads(Path("evals/reports/agent-latest.json").read_text(encoding="utf-8"))
        tests_replay = Path("tests/test_agent_eval_replay.py").read_text(encoding="utf-8")
        tests_normalizer = Path("tests/test_agent_recording_normalizer.py").read_text(encoding="utf-8")
        tests_suite = Path("tests/test_offline_eval_suite_agent.py").read_text(encoding="utf-8")

        self.assertTrue(Path("evals/golden/agent_predictions.v2.2.8.sample.jsonl").is_file())
        self.assertTrue(Path("evals/reports/agent-latest.md").is_file())
        self.assertIn("version-2.6.9-blue", readme)
        self.assertIn("## [2.2.8] - Agent Eval Replay & Stability", changelog)
        self.assertIn("### v2.2.8: Agent Eval Replay & Stability", readme)
        self.assertIn("docs/AGENT_EVAL.md", readme)
        self.assertIn("agent_prediction.schema.json", evals_readme)
        self.assertIn("agent_predictions.v2.2.8.sample.jsonl", evals_readme)
        self.assertIn("run_agent_eval.py --report-dir evals/reports --strict", evals_readme)
        self.assertIn("agent-latest.json", eval_reports)
        self.assertIn("evals/schemas/agent_prediction.schema.json", agent_eval)
        self.assertIn("run_offline_eval_suite.py --include-agent", agent_eval)
        self.assertIn("Agent Eval strict", status_doc)
        self.assertIn("run_agent_eval.py --report-dir evals/reports --strict", ci)
        self.assertIn("agent-latest.md", ci)
        self.assertIn("SCHEMA_VERSION = \"agent-eval-report.v1\"", runner)
        self.assertIn("compare_agent_baseline", runner)
        self.assertIn("--include-agent", suite_runner)
        self.assertIn("run_agent_report", suite_runner)
        self.assertIn("VOLATILE_KEYS", normalizer)
        self.assertIn("normalize_prediction", normalizer)
        self.assertIn("id", schema["required"])
        self.assertIn("usage", schema["properties"])
        self.assertEqual(baseline["version"], "2.2.8")
        self.assertEqual(agent_report["schemaVersion"], "agent-eval-report.v1")
        self.assertIn("test_agent_eval_replay_scores_normalized_predictions_and_writes_reports", tests_replay)
        self.assertIn("test_normalize_prediction_strips_volatile_fields_and_maps_aliases", tests_normalizer)
        self.assertIn("test_offline_eval_suite_can_include_agent_report_only_block", tests_suite)

    def test_v229_release_readiness_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        dockerfile = Path("Dockerfile").read_text(encoding="utf-8")
        ...
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        release = Path("scripts/release.py").read_text(encoding="utf-8")
        doctor_script = Path("scripts/doctor.py").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        runtime_doctor = Path("deepseek_infra/infra/diagnostics/runtime_doctor.py").read_text(encoding="utf-8")
        release_manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")

        self.assertTrue(Path("docs/RUNTIME_DOCTOR.md").is_file())
        self.assertTrue(Path("docs/RELEASE_READINESS.md").is_file())
        self.assertTrue(Path("tests/test_runtime_doctor.py").is_file())
        self.assertTrue(Path("tests/test_preflight_release.py").is_file())
        self.assertTrue(Path("tests/test_release_manifest.py").is_file())
        self.assertTrue(Path("tests/test_smoke_release.py").is_file())
        self.assertIn("version-2.6.9-blue", readme)
        self.assertIn('app_version: str = "2.6.9"', config)
        self.assertIn('versionName "2.6.9"', build_gradle)
        self.assertIn("versionCode 269", build_gradle)
        self.assertIn("deepseek-infra:2.6.9", dockerfile)
        self.assertIn("## [2.2.9] - Release Readiness & Runtime Doctor", changelog)
        self.assertIn("## [2.3.0] - Protocol Interop GA", changelog)
        self.assertIn("## [2.3.1] - GUI Interop Evidence Patch", changelog)
        self.assertIn("### v2.2.9: Release Readiness & Runtime Doctor", readme)
        self.assertIn("scripts/doctor.py --offline", readme)
        self.assertIn("scripts/preflight_release.py --version 2.6.9", readme)
        self.assertIn("scripts/smoke_release.py --offline", readme)
        self.assertIn("release-readiness", ci)
        self.assertIn("preflight_release.py --version 2.6.9", ci)
        self.assertIn("doctor.py --offline", ci)
        self.assertIn("--dry-run", release)
        self.assertIn("release_manifest", release)
        self.assertIn("--no-manifest", release)
        self.assertIn("run_doctor", doctor_script)
        self.assertIn("DoctorOptions", runtime_doctor)
        self.assertIn("STATUS_PASS", runtime_doctor)
        self.assertIn("mask_token", runtime_doctor)
        self.assertIn("run_preflight", preflight)
        self.assertIn("build_stages", smoke_release)
        self.assertIn("sha256_of", release_manifest)
        self.assertIn("SCHEMA_VERSION", release_manifest)

    def test_v232_headless_mcp_client_compatibility_pack_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        dockerfile = Path("Dockerfile").read_text(encoding="utf-8")
        ...
        headless_doc = Path("docs/integrations/headless-mcp-client.md").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_mcp_headless_bridge.py").read_text(encoding="utf-8")
        generator = Path("scripts/generate_mcp_client_config.py").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/headless-mcp-bridge.json").read_text(encoding="utf-8"))
        test_config_generator = Path("tests/test_mcp_config_generator.py").read_text(encoding="utf-8")
        test_headless_smoke = Path("tests/test_headless_mcp_bridge_smoke.py").read_text(encoding="utf-8")
        test_preflight = Path("tests/test_preflight_release.py").read_text(encoding="utf-8")
        ...
        self.assertIn("versionCode 269", build_gradle)
        self.assertIn("deepseek-infra:2.6.9", dockerfile)
        self.assertIn("## [2.3.2] - Headless MCP Client Compatibility Pack", changelog)
        self.assertIn("### v2.3.2: Headless MCP Client Compatibility Pack", readme)
        self.assertIn("scripts/smoke_mcp_headless_bridge.py --out docs/evidence/headless-mcp-bridge.json", ci)
        self.assertIn("preflight_release.py --version 2.6.9", ci)
        self.assertIn("Headless MCP bridge", compatibility)
        self.assertIn("Claude Desktop | ✅ GUI tested", compatibility)
        self.assertIn("Cursor | ✅ GUI tested", compatibility)
        self.assertIn("Headless MCP Evidence", readiness)
        self.assertIn("headless_mcp_bridge_evidence", preflight)
        self.assertIn("bridge.start", preflight)
        self.assertIn("stdio -> Streamable HTTP bridge", smoke)
        self.assertIn("stdio-bridge-child", smoke)
        self.assertIn("generate_config", generator)
        self.assertIn("mcp-remote", generator)
        self.assertIn("Client Config Generator", headless_doc)
        self.assertEqual(evidence["schemaVersion"], "headless-mcp-bridge-evidence.v1")
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertIn("mcp.policy_denial", {step["name"] for step in evidence["steps"]})
        self.assertIn("test_claude_stdio_bridge_config_uses_mcp_remote_header", test_config_generator)
        self.assertIn("test_headless_mcp_bridge_evidence_passes_without_token_leak", test_headless_smoke)
        self.assertIn("test_preflight_fails_on_missing_headless_mcp_evidence", test_preflight)


    def test_v233_a2a_external_peer_compatibility_pack_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        ...
        a2a_doc = Path("docs/integrations/a2a-external-peer.md").read_text(encoding="utf-8")
        third_party_plan = Path("docs/integrations/a2a-third-party-plan.md").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_a2a_external_peer.py").read_text(encoding="utf-8")
        schema = json.loads(Path("evals/schemas/a2a_external_peer_evidence.schema.json").read_text(encoding="utf-8"))
        evidence = json.loads(Path("docs/evidence/a2a-external-peer.json").read_text(encoding="utf-8"))
        langgraph_adapter = Path("examples/a2a_adapters/langgraph_peer_adapter.py").read_text(encoding="utf-8")
        crewai_adapter = Path("examples/a2a_adapters/crewai_peer_adapter.py").read_text(encoding="utf-8")
        test_smoke = Path("tests/test_a2a_external_peer_smoke.py").read_text(encoding="utf-8")
        test_evidence = Path("tests/test_a2a_external_peer_evidence.py").read_text(encoding="utf-8")
        test_preflight = Path("tests/test_preflight_release.py").read_text(encoding="utf-8")

        self.assertIn("## [2.3.3] - A2A External Peer Compatibility Pack", changelog)
        self.assertIn("### v2.3.3: A2A External Peer Compatibility Pack", readme)
        self.assertIn("scripts/smoke_a2a_external_peer.py --out docs/evidence/a2a-external-peer.json", ci)
        self.assertIn("A2A external peer smoke runner", compatibility)
        self.assertIn("Third-party A2A ecosystem peer", compatibility)
        self.assertIn("Third-party evidence tested", compatibility)
        self.assertIn("A2A External Peer Evidence", readiness)
        self.assertIn("A2A external peer evidence", implementation)
        self.assertIn("smoke_a2a_external_peer.py", a2a_doc)
        self.assertIn("examples/a2a_adapters/", third_party_plan)
        self.assertIn("check_a2a_external_peer_evidence", preflight)
        self.assertIn("check_a2a_third_party_peer_evidence", preflight)
        self.assertIn("summarize_stream_events", smoke)
        self.assertIn("tasks/list", smoke)
        self.assertEqual(schema["properties"]["checks"]["required"], list(evidence["checks"].keys()))
        self.assertEqual(evidence["schemaVersion"], "a2a-external-peer-evidence.v1")
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        self.assertTrue(all(value == "PASS" for value in evidence["checks"].values()))
        self.assertIn("run_langgraph_agent", langgraph_adapter)
        self.assertIn("run_crewai_crew", crewai_adapter)
        self.assertIn("test_artifact_chunk_order_must_be_sequential", test_smoke)
        self.assertIn("test_a2a_external_evidence_fails_when_required_check_is_missing", test_evidence)
        self.assertIn("test_preflight_fails_on_missing_a2a_external_peer_evidence", test_preflight)

    def test_v241_release_evidence_patch_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        dockerfile = Path("Dockerfile").read_text(encoding="utf-8")
        ...
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        release_manifest_module = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        headless_evidence = json.loads(Path("docs/evidence/headless-mcp-bridge.json").read_text(encoding="utf-8"))
        a2a_evidence = json.loads(Path("docs/evidence/a2a-external-peer.json").read_text(encoding="utf-8"))
        latest_report = json.loads(Path("evals/reports/latest.json").read_text(encoding="utf-8"))
        agent_report = json.loads(Path("evals/reports/agent-latest.json").read_text(encoding="utf-8"))
        compare_report = json.loads(Path("evals/reports/baseline-compare-latest.json").read_text(encoding="utf-8"))
        security_report = json.loads(Path("evals/reports/security-latest.json").read_text(encoding="utf-8"))
        test_encoding = Path("tests/test_docs_encoding_sanity.py").read_text(encoding="utf-8")
        test_index = Path("tests/test_evidence_index.py").read_text(encoding="utf-8")
        test_manifest = Path("tests/test_release_manifest.py").read_text(encoding="utf-8")
        ...
        self.assertIn("## [2.4.2] - GUI Interop Evidence Patch", changelog)
        self.assertIn("## [2.4.1] - Release Evidence Patch", changelog)
        ...
        self.assertIn("### v2.4.2: GUI Interop Evidence Patch", readme)
        self.assertIn("### v2.4.1: Release Evidence Patch", readme)
        self.assertIn("### v2.3.4: Release Evidence Polish & Encoding Fix", readme)
        self.assertIn("docs/EVIDENCE_INDEX.md", readme)
        self.assertIn("Headless MCP bridge", evidence_index)
        self.assertIn("A2A external peer", evidence_index)
        self.assertIn("Security corpus", evidence_index)
        self.assertIn("baseline-compare-latest.json", evidence_index)
        self.assertIn("docs_encoding_sanity", preflight)
        self.assertIn("quality_gate_evidence", preflight)
        self.assertIn("GARBLED_PATTERNS", preflight)
        self.assertIn("EVIDENCE_INDEX.md", preflight)
        self.assertIn("DEFAULT_EVIDENCE_PATHS", release_manifest_module)
        self.assertIn("DEFAULT_QUALITY_GATES", release_manifest_module)
        self.assertIn("docs/EVIDENCE_INDEX.md", release_manifest_module)
        self.assertEqual(headless_evidence["version"], "2.6.9")
        self.assertEqual(a2a_evidence["version"], "2.6.9")
        self.assertEqual(latest_report["version"], "2.6.9")
        self.assertEqual(agent_report["version"], "2.6.9")
        self.assertEqual(compare_report["version"], "2.6.9")
        self.assertEqual(security_report["version"], "2.6.9")
        for evidence in (headless_evidence, a2a_evidence, latest_report, agent_report, compare_report, security_report):
            self.assertIn("commit", evidence)
            self.assertIn("environment", evidence)
            self.assertIn("status", evidence)
            self.assertIn("os", evidence["environment"])
            self.assertIn("python", evidence["environment"])
            self.assertIn("ci", evidence["environment"])
        self.assertIn("test_docs_encoding_sanity", test_encoding)
        self.assertIn("test_evidence_index", test_index)
        self.assertIn("evidence", test_manifest)
        self.assertIn("preflight_release.py --version 2.6.9", ci)

    def test_v242_gui_interop_evidence_patch_is_present(self) -> None:
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        claude_doc = Path("docs/integrations/claude-desktop.md").read_text(encoding="utf-8")
        cursor_doc = Path("docs/integrations/cursor.md").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")

        self.assertIn("Claude Desktop | ✅ GUI tested", compatibility)
        self.assertIn("Cursor | ✅ GUI tested", compatibility)
        self.assertIn("commit `54228c4`", claude_doc)
        self.assertIn("commit `54228c4`", cursor_doc)
        self.assertIn("2026-06-28", claude_doc)
        self.assertIn("2026-06-28", cursor_doc)
        self.assertIn("Claude Desktop GUI", evidence_index)
        self.assertIn("Cursor GUI", evidence_index)
        self.assertIn("✅ GUI tested", evidence_index)
        self.assertIn("check_gui_interop_evidence", preflight)
        self.assertIn("## [2.4.2] - GUI Interop Evidence Patch", changelog)

    def test_v243_edge_router_evidence_patch_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        readiness = Path("docs/RELEASE_READINESS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        runbook = Path("docs/EDGE_ROUTER_RUNBOOK.md").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke = Path("examples/edge_router_smoke.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        schema = json.loads(Path("evals/schemas/edge_router_smoke_evidence.schema.json").read_text(encoding="utf-8"))
        evidence = json.loads(Path("docs/evidence/edge-router-smoke.json").read_text(encoding="utf-8"))

        self.assertIn("## [2.4.3] - Edge Router Evidence Patch", changelog)
        self.assertIn("### v2.4.3: Edge Router Evidence Patch", readme)
        self.assertIn("Edge Router smoke evidence", compatibility)
        self.assertIn("Experimental, with structured smoke evidence", implementation)
        self.assertIn("Edge Router Smoke Evidence（v2.4.3）", readiness)
        self.assertIn("edge_router_smoke_evidence", evidence_index)
        self.assertIn("--out docs/evidence/edge-router-smoke.json", runbook)
        self.assertIn("check_edge_router_smoke_evidence", preflight)
        self.assertIn("REQUIRED_CHECKS", smoke)
        self.assertIn("docs/evidence/edge-router-smoke.json", manifest)
        self.assertEqual(schema["properties"]["checks"]["required"], list(evidence["checks"].keys()))
        self.assertEqual(evidence["schemaVersion"], "edge-router-smoke-evidence.v1")
        self.assertEqual(evidence["version"], "2.6.0")
        self.assertEqual(evidence["status"], "PASS")

    def test_v245_continue_dev_mcp_compatibility_patch_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        readiness = Path("docs/RELEASE_READINESS.md").read_text(encoding="utf-8")
        ...
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        schema = json.loads(Path("evals/schemas/continue_dev_mcp_evidence.schema.json").read_text(encoding="utf-8"))
        evidence = json.loads(Path("docs/evidence/continue-dev-mcp.json").read_text(encoding="utf-8"))
        markdown = Path("docs/evidence/continue-dev-mcp.md").read_text(encoding="utf-8")
        test_preflight = Path("tests/test_preflight_release.py").read_text(encoding="utf-8")

        self.assertIn("## [2.4.5] - Continue.dev MCP Compatibility Patch", changelog)
        self.assertIn("### v2.4.5: Continue.dev MCP Compatibility Patch", readme)
        self.assertIn("Continue.dev", compatibility)
        self.assertIn("✅ Tested", compatibility)
        self.assertIn("Continue.dev", evidence_index)
        self.assertIn("Continue.dev MCP Evidence（v2.4.5）", readiness)
        self.assertIn("continue_dev_mcp_evidence", evidence_index)
        self.assertIn("Continue.dev MCP", continue_dev_doc)
        self.assertIn("Bearer token", continue_dev_doc)
        self.assertIn("check_continue_dev_mcp_evidence", preflight)
        self.assertIn("docs/evidence/continue-dev-mcp.json", manifest)
        self.assertEqual(schema["properties"]["checks"]["required"], list(evidence["checks"].keys()))
        self.assertEqual(evidence["schemaVersion"], "continue-dev-mcp-evidence.v1")
        self.assertEqual(evidence["version"], "2.6.0")
        self.assertEqual(evidence["status"], "PASS")
        self.assertTrue(all(value == "PASS" for value in evidence["checks"].values()))
        self.assertIn("# Continue.dev MCP Evidence", markdown)
        self.assertIn("test_preflight_fails_on_continue_dev_mcp_missing_required_check", test_preflight)
        self.assertIn("test_preflight_passes_on_continue_dev_mcp_evidence_complete", test_preflight)
        self.assertIn("test_preflight_warns_on_missing_continue_dev_mcp_evidence", test_preflight)

    def test_v246_openai_compatible_sdk_evidence_patch_is_present(self) -> None:
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        compatibility = Path("docs/COMPATIBILITY.md").read_text(encoding="utf-8")
        readiness = Path("docs/RELEASE_READINESS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_openai_compatible_sdks.py").read_text(encoding="utf-8")
        schema = json.loads(Path("evals/schemas/openai_compatible_sdks_evidence.schema.json").read_text(encoding="utf-8"))
        evidence = json.loads(Path("docs/evidence/openai-compatible-sdks.json").read_text(encoding="utf-8"))
        markdown = Path("docs/evidence/openai-compatible-sdks.md").read_text(encoding="utf-8")
        test_preflight = Path("tests/test_preflight_release.py").read_text(encoding="utf-8")
        test_smoke = Path("tests/test_smoke_openai_compatible_sdks.py").read_text(encoding="utf-8")
        reqs = Path("requirements-sdk-smoke.txt").read_text(encoding="utf-8")

        self.assertIn("## [2.4.6] - OpenAI-Compatible SDK Evidence Patch", changelog)
        self.assertIn("OpenAI-Compatible SDK Evidence", changelog)
        self.assertIn("✅ SDK smoke tested", compatibility)
        self.assertIn("openai_compatible_sdk_evidence", evidence_index)
        self.assertIn("OpenAI-Compatible SDK Evidence（v2.4.6）", readiness)
        self.assertIn("check_openai_compatible_sdk_evidence", preflight)
        self.assertIn("docs/evidence/openai-compatible-sdks.json", manifest)
        self.assertIn("langchain", smoke)
        self.assertIn("litellm", smoke)
        self.assertIn("llamaindex", smoke)
        self.assertIn("langchain-openai", reqs)
        self.assertEqual(schema["properties"]["sdks"]["required"], ["langchain", "litellm", "llamaindex"])
        self.assertEqual(evidence["schemaVersion"], "openai-compatible-sdks-evidence.v1")
        self.assertEqual(evidence["version"], "2.6.0")
        self.assertEqual(evidence["status"], "PASS")
        self.assertTrue(all(
            value == "PASS"
            for sdk_name in ("langchain", "litellm", "llamaindex")
            for value in evidence["sdks"][sdk_name].values()
        ))
        self.assertIn("# OpenAI-Compatible SDKs Smoke Evidence", markdown)
        self.assertIn("test_preflight_fails_on_openai_compatible_sdk_missing_required_check", test_preflight)
        self.assertIn("test_preflight_passes_on_openai_compatible_sdk_evidence_complete", test_preflight)
        self.assertIn("test_preflight_warns_on_missing_openai_compatible_sdk_evidence", test_preflight)
        self.assertIn("test_sdk_evidence_passes_with_all_required_checks", test_smoke)
        self.assertIn("test_sdk_evidence_schema_tracks_required_sdks", test_smoke)

    def test_v250_workspace_core_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        api = Path("docs/API.md").read_text(encoding="utf-8")
        workspace_doc = Path("docs/WORKSPACE.md").read_text(encoding="utf-8")
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        readiness = Path("docs/RELEASE_READINESS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        smoke_workspace = Path("scripts/smoke_workspace.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        workspace_evidence = json.loads(Path("docs/evidence/workspace-v2.6.9.json").read_text(encoding="utf-8"))
        test_workspace = Path("tests/test_workspace.py").read_text(encoding="utf-8")
        test_smoke_workspace = Path("tests/test_smoke_workspace.py").read_text(encoding="utf-8")

        self.assertIn("## [2.6.0] - Skill System", changelog)
        self.assertIn("### v2.6.0: Skill System", readme)
        self.assertIn("deepseek_infra/web/routes/files.py", readme)
        self.assertIn("deepseek_infra/web/routes/downloads.py", readme)
        self.assertIn("Workspace Core 工作台", readme)
        self.assertIn("infra/workspace/", readme)
        self.assertIn("docs/WORKSPACE.md", readme)
        self.assertIn("Workspace Core API", api)
        self.assertIn("/api/workspace/projects", api)
        self.assertIn("/api/workspace/exports", api)
        self.assertIn("Project 2.0", workspace_doc)
        self.assertIn("Saved Items", workspace_doc)
        self.assertIn("Artifact Hub", workspace_doc)
        self.assertIn("project-export.zip", workspace_doc)
        self.assertIn("Workspace Core | MVP", implementation)
        self.assertIn("Workspace Core Evidence（v2.6.9）", readiness)
        self.assertIn("workspace_core_evidence", evidence_index)
        self.assertIn("docs/evidence/workspace-v2.6.9.json", evidence_index)
        self.assertIn("check_workspace_core_evidence", preflight)
        self.assertIn("workspace_core", preflight)
        self.assertIn("workspace_core", smoke_release)
        self.assertIn("--skip-workspace", smoke_release)
        self.assertIn("projectExportZip", smoke_workspace)
        self.assertIn("secretRedaction", smoke_workspace)
        self.assertIn("workspaceCore", manifest)
        self.assertIn("docs/evidence/workspace-v2.6.9.json", manifest)
        self.assertIn("smoke_workspace.py --offline --out docs/evidence/workspace-v2.6.9.json", ci)
        self.assertEqual(workspace_evidence["version"], "2.6.9")
        self.assertEqual(workspace_evidence["status"], "PASS")
        for check in ("projectCreate", "savedItemCreate", "artifactList", "conversationExport", "projectExportZip", "secretRedaction"):
            self.assertEqual(workspace_evidence["checks"][check], "PASS")
        self.assertIn("test_workspace_project_saved_items_artifacts_and_zip_export", test_workspace)
        self.assertIn("test_workspace_smoke_writes_pass_evidence", test_smoke_workspace)

    def test_v263_custom_skill_builder_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        ...
        readiness = Path("docs/RELEASE_READINESS.md").read_text(encoding="utf-8")
        server = Path("deepseek_infra/web/server.py").read_text(encoding="utf-8")
        route = Path("deepseek_infra/web/routes/skills.py").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_skills = Path("scripts/smoke_skills.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/skills-v2.6.9.json").read_text(encoding="utf-8"))
        ui_evidence = json.loads(Path("docs/evidence/skills-ui-v2.6.9.json").read_text(encoding="utf-8"))
        builder_evidence = json.loads(Path("docs/evidence/skill-builder-v2.6.9.json").read_text(encoding="utf-8"))
        skill_eval = json.loads(Path("evals/reports/skill-latest.json").read_text(encoding="utf-8"))
        web_route_tests = Path("tests/test_web_skills_routes.py").read_text(encoding="utf-8")
        skills_js = Path("static/modules/skills.js").read_text(encoding="utf-8")
        chat_js = Path("static/modules/chat.js").read_text(encoding="utf-8")
        index = Path("static/index.html").read_text(encoding="utf-8")
        styles = Path("static/styles.css").read_text(encoding="utf-8")
        smoke_ui = Path("scripts/smoke_skills_ui.py").read_text(encoding="utf-8")
        smoke_builder = Path("scripts/smoke_skill_builder.py").read_text(encoding="utf-8")

        self.assertIn("## [2.6.3] - Custom Skill Builder", changelog)
        self.assertIn("### v2.6.3: Custom Skill Builder", readme)
        self.assertIn("Applicable version: v2.6.9.", skills_doc)
        self.assertIn("Skill Workbench UI", skills_doc)
        self.assertIn("Custom Skill Builder", skills_doc)
        self.assertIn("POST /api/skills/{skill_id}/run", skills_doc)
        self.assertIn("create_skills_router", server)
        self.assertIn("SkillsRouteDeps", route)
        self.assertIn("action == \"run\"", route)
        self.assertIn("action == \"validate\"", route)
        self.assertIn("action == \"dry_run\"", route)
        self.assertIn("skillApiRoutes", smoke_skills)
        self.assertIn("skill_system_evidence", preflight)
        self.assertIn("skill_ui_evidence", preflight)
        self.assertIn("skill_builder_evidence", preflight)
        self.assertIn("skillSystem", manifest)
        self.assertIn("skillWorkbench", manifest)
        self.assertIn("skillBuilder", manifest)
        self.assertIn("skill_system", smoke_release)
        self.assertIn("skill_workbench_ui", smoke_release)
        self.assertIn("skill_builder", smoke_release)
        self.assertIn("smoke_skills.py --offline --out docs/evidence/skills-v2.6.9.json", ci)
        self.assertIn("smoke_skills_ui.py --offline --out docs/evidence/skills-ui-v2.6.9.json", ci)
        self.assertIn("smoke_skill_builder.py --offline --out docs/evidence/skill-builder-v2.6.9.json", ci)
        self.assertIn("node --check static/modules/skills.js", ci)
        self.assertIn("docs/evidence/skills-v2.6.9.json", evidence_index)
        self.assertIn("docs/evidence/skills-ui-v2.6.9.json", evidence_index)
        self.assertIn("docs/evidence/skill-builder-v2.6.9.json", evidence_index)
        self.assertIn("Skill System Evidence（v2.6.9）", readiness)
        self.assertIn("Skill Workbench UI Evidence（v2.6.9）", readiness)
        self.assertIn("Skill Builder Evidence (v2.6.9)", readiness)
        self.assertIn("_env_path", config)
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        self.assertEqual(evidence["checks"]["skillApiRoutes"], "PASS")
        self.assertEqual(ui_evidence["version"], "2.6.9")
        self.assertEqual(ui_evidence["status"], "PASS")
        self.assertEqual(ui_evidence["checks"]["skillWorkbenchEntrypoint"], "PASS")
        self.assertEqual(ui_evidence["checks"]["skillRunSchemaForm"], "PASS")
        self.assertEqual(ui_evidence["checks"]["projectSkillBindingUi"], "PASS")
        self.assertEqual(builder_evidence["version"], "2.6.9")
        self.assertEqual(builder_evidence["status"], "PASS")
        self.assertEqual(builder_evidence["checks"]["builderOpen"], "PASS")
        self.assertEqual(builder_evidence["checks"]["cloneBuiltinSkill"], "PASS")
        self.assertEqual(builder_evidence["checks"]["visualInputSchemaEdit"], "PASS")
        self.assertEqual(builder_evidence["checks"]["toolPermissionPicker"], "PASS")
        self.assertEqual(builder_evidence["checks"]["offlineDryRun"], "PASS")
        self.assertEqual(builder_evidence["checks"]["skillBuilderAssets"], "PASS")
        self.assertTrue(Path("docs/assets/skill-workbench.png").is_file())
        self.assertTrue(Path("docs/assets/skill-run-result.png").is_file())
        self.assertTrue(Path("docs/assets/skill-builder.png").is_file())
        self.assertTrue(Path("docs/assets/skill-builder-dry-run.png").is_file())
        self.assertIn("renderRunFields", skills_js)
        self.assertIn("openSkillBuilder", skills_js)
        self.assertIn("buildSkillConfigFromBuilder", skills_js)
        self.assertIn("dryRunBuilderConfig", skills_js)
        self.assertIn("data-skill-clone", skills_js)
        self.assertNotIn("window.prompt", skills_js)
        self.assertIn("renderProjectSkillBinding", chat_js)
        self.assertIn('id="skillPanel"', index)
        self.assertIn('id="skillBuilderHost"', index)
        self.assertIn(".skill-panel", styles)
        self.assertIn(".skill-builder-host", styles)
        self.assertIn("skillUiAssets", smoke_ui)
        self.assertIn("visualInputSchemaEdit", smoke_builder)
        self.assertEqual(skill_eval["version"], "2.6.9")
        self.assertEqual(skill_eval["status"], "PASS")
        self.assertIn("test_skills_action_list_get_create_disable_enable_and_run", web_route_tests)
        self.assertIn("test_skills_validate_and_dry_run_authoring_actions", web_route_tests)

    def test_v264_skill_packs_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        skills_doc = Path("docs/SKILLS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        readiness = Path("docs/RELEASE_READINESS.md").read_text(encoding="utf-8")
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        frontend = Path("docs/FRONTEND_MODULES.md").read_text(encoding="utf-8")
        pack_module = Path("deepseek_infra/infra/skills/pack.py").read_text(encoding="utf-8")
        registry = Path("deepseek_infra/infra/skills/registry.py").read_text(encoding="utf-8")
        route = Path("deepseek_infra/web/routes/skills.py").read_text(encoding="utf-8")
        workspace_route = Path("deepseek_infra/web/routes/workspace.py").read_text(encoding="utf-8")
        projects = Path("deepseek_infra/infra/data/projects.py").read_text(encoding="utf-8")
        config = Path("deepseek_infra/core/config.py").read_text(encoding="utf-8")
        server = Path("deepseek_infra/web/server.py").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        smoke_packs = Path("scripts/smoke_skill_packs.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        skills_js = Path("static/modules/skills.js").read_text(encoding="utf-8")
        index = Path("static/index.html").read_text(encoding="utf-8")
        styles = Path("static/styles.css").read_text(encoding="utf-8")
        build_gradle = Path("android/app/build.gradle").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/skill-packs-v2.6.9.json").read_text(encoding="utf-8"))
        pack_tests = Path("tests/test_skill_packs.py").read_text(encoding="utf-8")

        self.assertIn("## [2.6.4] - Skill Packs", changelog)
        self.assertIn("### v2.6.4: Skill Packs", readme)
        self.assertIn("Skill Packs", skills_doc)
        self.assertIn("packId", skills_doc)
        self.assertIn("Study Pack", skills_doc)
        self.assertIn("onConflict", skills_doc)
        self.assertIn("validate_pack_config", pack_module)
        self.assertIn("tool_permission_summary", pack_module)
        self.assertIn("high_risk_tools", pack_module)
        self.assertIn("def list_packs", registry)
        self.assertIn("def import_pack", registry)
        self.assertIn("def export_pack", registry)
        self.assertIn('action == "list_packs"', route)
        self.assertIn('action == "import_pack"', route)
        self.assertIn('action == "export_pack"', route)
        self.assertIn('action == "validate_pack"', route)
        self.assertIn('action == "delete_pack"', route)
        self.assertIn("skill-packs/{pack_id}/install", workspace_route)
        self.assertIn("enable_pack_for_project", projects)
        self.assertIn("enabledPacks", projects)
        self.assertIn("BUILTIN_PACKS_DIR", config)
        self.assertIn("list_packs=", server)
        self.assertIn("skill_packs_evidence", preflight)
        self.assertIn("skill_packs", smoke_release)
        self.assertIn("skillPacks", manifest)
        self.assertIn("skill-packs-v2.6.9.json", manifest)
        self.assertIn("smoke_skill_packs.py --offline --out docs/evidence/skill-packs-v2.6.9.json", ci)
        self.assertIn("node --check static/modules/skills.js", ci)
        self.assertIn("openPacksHost", skills_js)
        self.assertIn("renderPackCard", skills_js)
        self.assertIn("importPackFromFile", skills_js)
        self.assertIn("installPack", skills_js)
        self.assertIn('id="skillPacksButton"', index)
        self.assertIn('id="skillPacksHost"', index)
        self.assertIn('id="skillBuiltinPackList"', index)
        self.assertIn('id="skillCustomPackList"', index)
        self.assertIn(".skill-packs-host", styles)
        self.assertIn(".skill-pack-card", styles)
        self.assertIn('versionName "2.6.9"', build_gradle)
        self.assertIn("versionCode 269", build_gradle)
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        self.assertEqual(evidence["checks"]["packSchemaValidation"], "PASS")
        self.assertEqual(evidence["checks"]["builtinPacksLoad"], "PASS")
        self.assertEqual(evidence["checks"]["skillIdConflictHandling"], "PASS")
        self.assertEqual(evidence["checks"]["projectPackBinding"], "PASS")
        self.assertTrue(Path("docs/assets/skill-packs.png").is_file())
        self.assertTrue(Path("docs/assets/skill-pack-import.png").is_file())
        self.assertTrue(Path("skills/packs/study.json").is_file())
        self.assertTrue(Path("skills/packs/research.json").is_file())
        self.assertTrue(Path("skills/packs/code.json").is_file())
        self.assertTrue(Path("skills/packs/office.json").is_file())
        self.assertIn("docs/evidence/skill-packs-v2.6.9.json", evidence_index)
        self.assertIn("scripts/smoke_skill_packs.py", evidence_index)
        self.assertIn("Skill Packs Evidence (v2.6.9)", readiness)
        self.assertIn("Skill Packs Evidence", implementation)
        self.assertIn("openPacksHost", frontend)
        self.assertTrue(Path("scripts/smoke_skill_packs.py").is_file())
        self.assertIn("Skill Packs", smoke_packs)
        self.assertIn("packSchemaValidation", smoke_packs)
        self.assertIn("test_import_pack_creates_skills_and_manifest", pack_tests)
        self.assertIn("test_enable_pack_for_project_enables_skills", pack_tests)

    def test_v265_skill_eval_dashboard_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        skills_doc = Path("docs/SKILLS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        readiness = Path("docs/RELEASE_READINESS.md").read_text(encoding="utf-8")
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        eval_module = Path("deepseek_infra/infra/skills/eval.py").read_text(encoding="utf-8")
        route = Path("deepseek_infra/web/routes/skills.py").read_text(encoding="utf-8")
        runner = Path("evals/runners/run_skill_eval.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_skill_eval_dashboard.py").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        ...
        styles = Path("static/styles.css").read_text(encoding="utf-8")
        web_route_tests = Path("tests/test_web_skills_routes.py").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/skill-eval-dashboard-v2.6.9.json").read_text(encoding="utf-8"))
        report = json.loads(Path("evals/reports/skills-v2.6.9.json").read_text(encoding="utf-8"))

        self.assertIn("## [2.6.5] - Skill Eval Dashboard", changelog)
        self.assertIn("### v2.6.5: Skill Eval Dashboard", readme)
        self.assertIn("Skill Eval Dashboard", skills_doc)
        self.assertIn("Eval Case Builder", skills_doc)
        self.assertIn("Skill eval report is `evals/reports/skills-v2.6.9.json`", skills_doc)
        self.assertIn("build_skill_eval_report", eval_module)
        self.assertIn("compare_reports", eval_module)
        self.assertIn("save_eval_case", eval_module)
        self.assertIn('action == "eval_report"', route)
        self.assertIn('action == "create_eval_case"', route)
        self.assertIn('action == "delete_eval_case"', route)
        self.assertIn("--scope", runner)
        self.assertIn("--baseline", runner)
        self.assertIn("markdown_summary", runner)
        self.assertIn("skill_eval_dashboard_evidence", preflight)
        self.assertIn("skill_eval_dashboard", smoke_release)
        self.assertIn("skillEvalDashboard", manifest)
        self.assertIn("evals/reports/skills-v2.6.9.json", manifest)
        self.assertIn("smoke_skill_eval_dashboard.py --offline --out docs/evidence/skill-eval-dashboard-v2.6.9.json", ci)
        self.assertIn("runSkillEval", skills_js)
        self.assertIn("renderEvalDashboard", skills_js)
        self.assertIn("saveSkillEvalCase", skills_js)
        self.assertIn("exportSkillEvalJson", skills_js)
        self.assertIn('id="skillEvalButton"', index)
        self.assertIn('id="skillEvalHost"', index)
        self.assertIn('id="skillEvalCaseForm"', index)
        self.assertIn(".skill-eval-host", styles)
        self.assertIn(".skill-eval-case-form", styles)
        self.assertIn("evalDashboardEntrypoint", smoke)
        self.assertIn("regressionCompare", smoke)
        self.assertIn("docs/evidence/skill-eval-dashboard-v2.6.9.json", evidence_index)
        self.assertIn("Skill Eval Dashboard Evidence", readiness)
        self.assertIn("Skill Eval Dashboard Evidence", implementation)
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        self.assertEqual(evidence["checks"]["evalDashboardEntrypoint"], "PASS")
        self.assertEqual(evidence["checks"]["evalCaseBuilder"], "PASS")
        self.assertEqual(evidence["checks"]["packLevelEval"], "PASS")
        self.assertEqual(report["version"], "2.6.9")
        self.assertEqual(report["status"], "PASS")
        self.assertGreaterEqual(report["summary"]["skillCount"], 6)
        self.assertGreaterEqual(report["summary"]["packCount"], 4)
        self.assertEqual(report["checks"]["packLevelEval"], "PASS")
        self.assertTrue(Path("docs/assets/skill-eval-dashboard.png").is_file())
        self.assertTrue(Path("docs/assets/skill-eval-case-builder.png").is_file())
        self.assertIn("test_skills_eval_report_and_case_builder_actions", web_route_tests)

    def test_v266_skill_versioning_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        skills_doc = Path("docs/SKILLS.md").read_text(encoding="utf-8")
        frontend = Path("docs/FRONTEND_MODULES.md").read_text(encoding="utf-8")
        versioning_module = Path("deepseek_infra/infra/skills/versioning.py").read_text(encoding="utf-8")
        registry = Path("deepseek_infra/infra/skills/registry.py").read_text(encoding="utf-8")
        route = Path("deepseek_infra/web/routes/skills.py").read_text(encoding="utf-8")
        workspace_route = Path("deepseek_infra/web/routes/workspace.py").read_text(encoding="utf-8")
        projects = Path("deepseek_infra/infra/data/projects.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_skill_versioning.py").read_text(encoding="utf-8")
        skills_js = Path("static/modules/skills.js").read_text(encoding="utf-8")
        index = Path("static/index.html").read_text(encoding="utf-8")
        styles = Path("static/styles.css").read_text(encoding="utf-8")
        tests = Path("tests/test_skills.py").read_text(encoding="utf-8")
        web_route_tests = Path("tests/test_web_skills_routes.py").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/skill-versioning-v2.6.9.json").read_text(encoding="utf-8"))

        self.assertIn("## [2.6.6] - Skill Versioning & Migration", changelog)
        self.assertIn("### v2.6.6: Skill Versioning & Migration", readme)
        self.assertIn("Skill Versioning & Migration", skills_doc)
        self.assertIn("versioning.py", skills_doc)
        self.assertIn("list_versions", skills_doc)
        self.assertIn("enabledPackVersions", skills_doc)
        self.assertIn("Skill Versioning", frontend)
        self.assertIn("def snapshot_skill", versioning_module)
        self.assertIn("def diff_skill_versions", versioning_module)
        self.assertIn("def migration_plan", versioning_module)
        self.assertIn("def rollback_skill", versioning_module)
        self.assertIn("def upgrade_pack", versioning_module)
        self.assertIn("eval_aware_upgrade_gate", versioning_module)
        self.assertIn("snapshot_skill", registry)
        self.assertIn("snapshot_pack", registry)
        self.assertIn('action == "list_versions"', route)
        self.assertIn('action == "diff_versions"', route)
        self.assertIn('action == "rollback_skill"', route)
        self.assertIn('action == "migration_plan"', route)
        self.assertIn('action == "upgrade_pack"', route)
        self.assertIn("enabledPackVersions", workspace_route)
        self.assertIn("normalize_project_pack_versions", projects)
        self.assertIn("openVersionHost", skills_js)
        self.assertIn("loadSkillVersions", skills_js)
        self.assertIn("rollbackSkillVersion", skills_js)
        self.assertIn("runPackUpgradeGate", skills_js)
        self.assertIn('id="skillVersionsButton"', index)
        self.assertIn('id="skillVersionsHost"', index)
        self.assertIn(".skill-version-host", styles)
        self.assertIn("skillVersionSnapshot", smoke)
        self.assertIn("evalAwareUpgradeGate", smoke)
        self.assertIn("test_skill_version_history_diff_migration_and_rollback", tests)
        self.assertIn("test_skills_versioning_actions", web_route_tests)
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        for check in (
        ...
        self.assertTrue(Path("docs/assets/skill-version-history.png").is_file())
        self.assertTrue(Path("docs/assets/skill-version-diff.png").is_file())

    def test_v267_skill_run_analytics_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        skills_doc = Path("docs/SKILLS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        ...
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        frontend = Path("docs/FRONTEND_MODULES.md").read_text(encoding="utf-8")
        analytics_module = Path("deepseek_infra/infra/skills/analytics.py").read_text(encoding="utf-8")
        runner = Path("deepseek_infra/infra/skills/runner.py").read_text(encoding="utf-8")
        route = Path("deepseek_infra/web/routes/skills.py").read_text(encoding="utf-8")
        workspace_route = Path("deepseek_infra/web/routes/workspace.py").read_text(encoding="utf-8")
        projects = Path("deepseek_infra/infra/data/projects.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_skill_analytics.py").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        ...
        index = Path("static/index.html").read_text(encoding="utf-8")
        styles = Path("static/styles.css").read_text(encoding="utf-8")
        analytics_tests = Path("tests/test_skill_analytics.py").read_text(encoding="utf-8")
        web_route_tests = Path("tests/test_web_skills_routes.py").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/skill-analytics-v2.6.9.json").read_text(encoding="utf-8"))

        self.assertIn("## [2.6.7] - Skill Run Analytics", changelog)
        self.assertIn("### v2.6.7: Skill Run Analytics", readme)
        self.assertIn("Skill Run Analytics", skills_doc)
        self.assertIn("analytics.py", skills_doc)
        self.assertIn("Skill Analytics Evidence", readiness)
        self.assertIn("Skill Analytics Evidence", implementation)
        self.assertIn("Skill Run Analytics", frontend)
        self.assertIn("def record_success", analytics_module)
        self.assertIn("def record_failure", analytics_module)
        self.assertIn("def analytics_summary", analytics_module)
        self.assertIn("def cleanup_runs", analytics_module)
        self.assertIn("record_success", runner)
        self.assertIn("record_failure", runner)
        for action in (
            'action == "list_runs"',
            'action == "get_run"',
            'action == "delete_run"',
            ...
            self.assertIn(action, route)
        self.assertIn("skill-analytics", workspace_route)
        self.assertIn("failureCategory", projects)
        self.assertIn("skill_analytics_evidence", preflight)
        self.assertIn("skill_analytics", smoke_release)
        self.assertIn("skillAnalytics", manifest)
        self.assertIn("docs/evidence/skill-analytics-v2.6.9.json", manifest)
        self.assertIn("smoke_skill_analytics.py --offline --out docs/evidence/skill-analytics-v2.6.9.json", ci)
        self.assertIn("openRunsHost", skills_js)
        self.assertIn("loadRunsDashboard", skills_js)
        self.assertIn("cleanupFailedRuns", skills_js)
        self.assertIn("redactSkillRun", skills_js)
        self.assertIn('id="skillRunsButton"', index)
        self.assertIn('id="skillRunsHost"', index)
        self.assertIn(".skill-runs-host", styles)
        self.assertIn(".skill-run-analytics-row", styles)
        self.assertIn("skillRunHistory", smoke)
        self.assertIn("privacyRedaction", smoke)
        self.assertIn("docs/evidence/skill-analytics-v2.6.9.json", evidence_index)
        self.assertIn("test_skill_run_analytics_records_success_failure_summary_and_redaction", analytics_tests)
        self.assertIn("test_skills_run_analytics_actions_and_project_endpoint", web_route_tests)
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        for check in (
            "skillRunHistory",
            ...
            self.assertEqual(evidence["checks"][check], "PASS")
        self.assertTrue(Path("docs/assets/skill-runs.png").is_file())
        self.assertTrue(Path("docs/assets/skill-analytics.png").is_file())

    def test_v268_skill_security_review_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        skills_doc = Path("docs/SKILLS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        ...
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        frontend = Path("docs/FRONTEND_MODULES.md").read_text(encoding="utf-8")
        security_module = Path("deepseek_infra/infra/skills/security.py").read_text(encoding="utf-8")
        runner = Path("deepseek_infra/infra/skills/runner.py").read_text(encoding="utf-8")
        analytics = Path("deepseek_infra/infra/skills/analytics.py").read_text(encoding="utf-8")
        route = Path("deepseek_infra/web/routes/skills.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_skill_security.py").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        ...
        index = Path("static/index.html").read_text(encoding="utf-8")
        styles = Path("static/styles.css").read_text(encoding="utf-8")
        security_tests = Path("tests/test_skill_security.py").read_text(encoding="utf-8")
        web_route_tests = Path("tests/test_web_skills_routes.py").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/skill-security-v2.6.9.json").read_text(encoding="utf-8"))

        self.assertIn("## [2.6.8] - Skill Security Review", changelog)
        self.assertIn("### v2.6.8: Skill Security Review", readme)
        self.assertIn("Skill Security Review", skills_doc)
        self.assertIn("security.py", skills_doc)
        self.assertIn("Skill Security Evidence", readiness)
        self.assertIn("Skill Security Evidence", implementation)
        self.assertIn("Security 面板", frontend)
        self.assertIn("SECURITY_REVIEW_SCHEMA", security_module)
        self.assertIn("SECURITY_MANIFEST_SCHEMA", security_module)
        self.assertIn("def review_skill", security_module)
        self.assertIn("def review_pack", security_module)
        self.assertIn("def security_summary", security_module)
        self.assertIn("def trust_skill", security_module)
        self.assertIn("def block_skill", security_module)
        self.assertIn("def tool_grant_review", security_module)
        self.assertIn("security_context_for_skill", runner)
        self.assertIn("security_metadata", runner)
        self.assertIn("runSecurityLevel", analytics)
        for action in (
            ...
            self.assertIn(action, route)
        self.assertIn("skill_security_evidence", preflight)
        self.assertIn("skill_security", smoke_release)
        self.assertIn("skillSecurity", manifest)
        self.assertIn("docs/evidence/skill-security-v2.6.9.json", manifest)
        self.assertIn("smoke_skill_security.py --offline --out docs/evidence/skill-security-v2.6.9.json", ci)
        self.assertIn("openSecurityHost", skills_js)
        self.assertIn("loadSecurityDashboard", skills_js)
        self.assertIn("setSkillTrust", skills_js)
        self.assertIn('id="skillSecurityButton"', index)
        self.assertIn('id="skillSecurityHost"', index)
        self.assertIn(".skill-security-host", styles)
        self.assertIn(".skill-security-row", styles)
        self.assertIn("securityReview", smoke)
        self.assertIn("tamperDetection", smoke)
        self.assertIn("runSecurityMetadata", smoke)
        self.assertIn("docs/evidence/skill-security-v2.6.9.json", evidence_index)
        self.assertIn("test_skill_security_review_scans_prompt_tools_and_manifest", security_tests)
        self.assertIn("test_skills_security_review_actions", web_route_tests)
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        for check in (
            "securityReview",
            ...
            self.assertEqual(evidence["checks"][check], "PASS")
        self.assertTrue(Path("docs/assets/skill-security-review.png").is_file())
        self.assertTrue(Path("docs/assets/skill-trust-store.png").is_file())

    def test_v269_local_skill_catalog_is_present(self) -> None:
        readme = Path("README.md").read_text(encoding="utf-8")
        changelog = Path("CHANGELOG.md").read_text(encoding="utf-8")
        skills_doc = Path("docs/SKILLS.md").read_text(encoding="utf-8")
        evidence_index = Path("docs/EVIDENCE_INDEX.md").read_text(encoding="utf-8")
        ...
        implementation = Path("docs/IMPLEMENTATION_STATUS.md").read_text(encoding="utf-8")
        frontend = Path("docs/FRONTEND_MODULES.md").read_text(encoding="utf-8")
        catalog_module = Path("deepseek_infra/infra/skills/catalog.py").read_text(encoding="utf-8")
        route = Path("deepseek_infra/web/routes/skills.py").read_text(encoding="utf-8")
        smoke = Path("scripts/smoke_skill_catalog.py").read_text(encoding="utf-8")
        preflight = Path("scripts/preflight_release.py").read_text(encoding="utf-8")
        smoke_release = Path("scripts/smoke_release.py").read_text(encoding="utf-8")
        manifest = Path("deepseek_infra/infra/diagnostics/release_manifest.py").read_text(encoding="utf-8")
        ci = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
        ...
        index = Path("static/index.html").read_text(encoding="utf-8")
        styles = Path("static/styles.css").read_text(encoding="utf-8")
        catalog_tests = Path("tests/test_skill_catalog.py").read_text(encoding="utf-8")
        web_route_tests = Path("tests/test_web_skills_routes.py").read_text(encoding="utf-8")
        evidence = json.loads(Path("docs/evidence/skill-catalog-v2.6.9.json").read_text(encoding="utf-8"))

        self.assertIn("## [2.6.9] - Local Skill Catalog", changelog)
        self.assertIn("### v2.6.9: Local Skill Catalog", readme)
        self.assertIn("Local Skill Catalog", skills_doc)
        self.assertIn("catalog.py", skills_doc)
        self.assertIn("Skill Catalog Evidence", readiness)
        self.assertIn("Skill Catalog Evidence", implementation)
        self.assertIn("Catalog", frontend)
        for marker in (
            ...
            self.assertIn(marker, catalog_module)
        for action in (
            'action == "catalog_list"',
            'action == "catalog_get"',
            ...
            self.assertIn(action, route)
        self.assertIn("skill_catalog_evidence", preflight)
        self.assertIn("skill_catalog", smoke_release)
        self.assertIn("skillCatalog", manifest)
        self.assertIn("docs/evidence/skill-catalog-v2.6.9.json", manifest)
        self.assertIn("smoke_skill_catalog.py --offline --out docs/evidence/skill-catalog-v2.6.9.json", ci)
        self.assertIn("openCatalogHost", skills_js)
        self.assertIn("loadCatalogDashboard", skills_js)
        self.assertIn("previewCatalogInstall", skills_js)
        self.assertIn("installCatalogItem", skills_js)
        self.assertIn('id="skillCatalogButton"', index)
        self.assertIn('id="skillCatalogHost"', index)
        self.assertIn(".skill-catalog-host", styles)
        self.assertIn(".skill-catalog-row", styles)
        self.assertIn("catalogManifest", smoke)
        self.assertIn("securityGateBeforeInstall", smoke)
        self.assertIn("toolPermissionSummary", smoke)
        self.assertIn("docs/evidence/skill-catalog-v2.6.9.json", evidence_index)
        self.assertIn("test_catalog_manifest_list_search_and_export", catalog_tests)
        self.assertIn("test_skills_catalog_actions", web_route_tests)
        self.assertEqual(evidence["version"], "2.6.9")
        self.assertEqual(evidence["status"], "PASS")
        for check in (
            "catalogManifest",
            ...
            self.assertEqual(evidence["checks"][check], "PASS")
        self.assertTrue(Path("docs/assets/skill-catalog.png").is_file())
        self.assertTrue(Path("docs/assets/skill-catalog-install-preview.png").is_file())
