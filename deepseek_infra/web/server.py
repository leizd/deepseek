...
def create_app() -> FastAPI:
    ...
    @api.get("/api/workspace/projects/{project_id}")
    ...
    @api.patch("/api/workspace/projects/{project_id}")
    ...
    @api.delete("/api/workspace/projects/{project_id}")
    ...
    @api.get("/api/workspace/projects/{project_id}/conversations")
    ...
    @api.post("/api/workspace/projects/{project_id}/conversations")
    ...
    @api.get("/api/workspace/projects/{project_id}/saved-items")
    ...
    @api.post("/api/workspace/projects/{project_id}/saved-items")
    ...
    @api.patch("/api/workspace/projects/{project_id}/saved-items/{saved_id}")
    ...
    @api.delete("/api/workspace/projects/{project_id}/saved-items/{saved_id}")
    ...
    @api.get("/api/workspace/projects/{project_id}/artifacts")
    ...
    @api.post("/api/workspace/projects/{project_id}/artifacts")
    ...
    @api.patch("/api/workspace/projects/{project_id}/artifacts/{artifact_id}")
    ...
    @api.delete("/api/workspace/projects/{project_id}/artifacts/{artifact_id}")
    ...
    @api.get("/api/workspace/artifacts/{artifact_id}/preview")
    async def api_workspace_artifact_preview(request: Request, artifact_id: str) -> JSONResponse:
        require_api_auth(request)
        return json_response({"ok": True, **workspace_artifacts.preview_artifact(artifact_id, project_id=str(request.query_params.get("projectId") or ""))})

    @api.get("/api/workspace/artifacts/{artifact_id}/download")
    async def api_workspace_artifact_download(request: Request, artifact_id: str) -> Response:
        ...
        headers = {"Content-Disposition": content_disposition_header("attachment", workspace_artifacts.artifact_filename(artifact)), "Cache-Control": "no-store"}
    ...
    @api.get("/api/workspace/exports/{export_id}/download")
    async def api_workspace_export_download(request: Request, export_id: str) -> Response:
        ...
        data = path.read_bytes()
        media_type = mimetypes.guess_type(str(path))[0] or "application/octet-stream"
        headers = {"Content-Disposition": content_disposition_header("attachment", str(export.get("filename") or path.name)), "Cache-Control": "no-store"}
        return Response(content=data, media_type=media_type, headers=headers)
...
def project_action(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "get":
        return {"ok": True, "project": workspace_projects.get_project(str(payload.get("id") or payload.get("projectId") or ""))}
    if action == "rename":
        return {
            "ok": True,
            "project": workspace_projects.rename_project(
                str(payload.get("id") or payload.get("projectId") or ""),
                str(payload.get("name") or ""),
                description=str(payload.get("description")) if "description" in payload else None,
            ),
        }
    if action == "delete":
        return {"ok": True, "deleted": delete_project(str(payload.get("id") or payload.get("projectId") or ""))}
